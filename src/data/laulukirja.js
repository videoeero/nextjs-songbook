export default [
  {
    metadata: ['(Säv. Tip tap)', 'Tunnetaan myös nimellä: Infåiternas julvisa'],
    number: '101',
    lyrics: [
      '1, 0, 1, 0 ',
      '10101, 0,',
      '1,',
      '0,',
      '1,',
      '0',
      '1, 0, 1, 0',
      '10101, 0,',
      '1,',
      '0,',
      '1,',
      '0',
      '1, 0, 1, 0',
      '10101, 0,',
      '1,',
      '1,',
      '0.'
    ],
    title: '1, 0'
  },
  {
    metadata: ['(Säv. Sankarit, alkup. esit. J. Karjalainen)'],
    number: '102',
    lyrics: [
      'Keitä ne on, ne asteriskit,',
      'sellaiset asteriskimiehet,',
      'joita koko yliopisto arvostaa?',
      'Keitä ne on, ne asteriskit,',
      'sellaiset asteriskinaiset,',
      'jotka tosityöstä palkintonsa saa?',
      '>>__KERTO:__',
      '>>Me ollaan asteriskeja kaikki,',
      '>>kun oikein silmin katsotaan',
      '>>Me ollaan asteriskeja Turusta,',
      '>> ihan jokainen',
      'Keitä ne on, ne asteriskit,',
      'sellaiset asteriskinaiset,',
      'jotka tosipaikan tullen koodaavat?',
      'Keitä ne on, ne asteriskit,',
      'sellaiset asteriskimiehet,',
      'jotka rahaa tukilautakunnalta kerjäävät?',
      '>>__KERTO__',
      'Keitä ne on, ne asteriskit,',
      'ICT:n asteriskinaiset,',
      'jotka kauneudellaan kaikki lumoaa?',
      'Keitä ne on, ne asteriskit,',
      'ICT:n asteriskimiehet,',
      'jotka tietokoneet tieltänsä kumoaa?',
      '>>_KERTO X 2_'
    ],
    title: 'Asteriskit'
  },
  {
    metadata: ['(Säv. Apinaorkesteri, alkup. esit. J. Karjalainen)'],
    number: '103',
    lyrics: [
      '>>__KERTO:__',
      '>>As-te-riski-orkesteri.',
      '>>DataCityn suojassa harjoitteli.',
      '>>As-te-riski-orkesteri.',
      ">>hetken aikaa meitä 'viihdytti'!",
      'Suurin asteriski oli solisti,',
      'se lauloi ja näyttöpäätettä kolisti.',
      'Myös pienin tunsi rytmin luissaan,',
      'se mukana tanssi asteriskihaalareissaan.',
      'Se oli',
      '>>__KERTO__',
      'Tyttö näppäimistöä heilutti',
      'ja keskusyksiköllä itseään keinutti.',
      'Laiha asteriski soitti modeemilla,',
      'ei tiennyt mitään muuta sen hauskempaa.',
      'Se oli',
      '>>KERTO'
    ],
    title: 'Asteriskiorkesteri'
  },
  {
    metadata: [],
    number: '104',
    lyrics: [
      'SING',
      'Yksi pieni elefantti marssi näin,',
      'aurinkoista tietä eteenpäin',
      'Koska matka oli hauska niin,',
      'pyysi hän mukaan yhden toverin',
      'N := 2;',
      'REPEAT',
      'SING',
      'N pientä elefanttia marssi näin,',
      'aurinkoista tietä eteenpäin',
      'Koska matka oli hauska niin,',
      'pyysivät he mukaan yhden toverin',
      'N := N + 1;',
      'UNTIL N = 25 OR BLACKOUT = TRUE;'
    ],
    title: 'Carlsbergin juomalaulu'
  },
  {
    metadata: ['(Säv. Mors lilla grisar)'],
    number: '106',
    lyrics: [
      'Icen pienet riskit ryyppää viinaa,',
      'ryyppää viinaa, ryyppää viinaa.',
      'Icen pienet riskit ryyppää viinaa,',
      'ryyppää viinaa, viinaa.',
      'Icen pienet riskit tulee känniin...',
      'Icen pienet riskit sitten sammuu...',
      'Icen pienet riskit joutuu putkaan...',
      'Icen pienet riskit rikkoo putkan...',
      'Icen pienet riskit menee metsään...',
      'Icen pienet riskit tekee tehtaan...',
      'Icen pienet riskit keittää viinaa...',
      'Icen pienet riskit ryyppää viinaa...',
      '_REKURSION OPPITUNTI_'
    ],
    title: 'ICT:n pienet riskit'
  },
  {
    metadata: [
      '(Säv. Seasons in the Sun)',
      'Omistettu *:n toimiston auringolle'
    ],
    number: '105',
    lyrics: [
      "Goodbye to my shell, it's hard to vi,",
      'I cannot socket(), even though I try,',
      'everything keeps going wrong...',
      'It needs a bind() to carry on,',
      "Proc' table's been full for too long",
      '>>We had joy, we had fun,',
      '>>we were fork()ing on a Sun',
      '>>But the joy is all gone,',
      ">>_'til the processes are Done[1]_"
    ],
    title: 'Fork()ing on a Sun'
  },
  {
    metadata: ['(Säv. Baarikärpänen, alkup. esit. Eppu Normaali)'],
    number: '107',
    lyrics: [
      'Mua puri nettikärpänen, mut miksi suremaan?',
      'Jo kauan sitten lääkkeen löysin siihen puremaan.',
      'Alla tietokonepäätteen onnellisten ledien',
      'on hyvä olo ollut mulla siitä lähtien',
      'Jos silmät ei oo neliöt, niin missä vika on?',
      'No atk-keskushan on ollut vastuuton!',
      'Kas, data menee ensin silmiin, sieltä aivoihin,',
      'ja sieltä käsin vaikuttaa se kaikkiin vaivoihin.',
      '>>__KERTO:__',
      '>>Siis surffaamaan, surffaamaan joka ilta sännätään',
      '>>ja kun aamu on ohi, niin lisää surffataan!',
      '>>Tahdon olla internetissä aina sekä perjantaina,',
      '>>lauantaina, sunnuntaina ja ainiaan!',
      'Assareidenkin tiedetään mua mätkineen.',
      'Ne oottaa mua käytävällä nettikärpäslätkineen.',
      'Voi, jospa voisin jäädä tänne mikroluokkaan nukkumaan,',
      'kun multa ihmisoikeudet on näemmä päässyt hukkumaan.',
      '>>__KERTO__',
      'Ne jossain pohtii nytkin modeemimaksun nostoa.',
      'Mä istun hiiri kädessä janoten kostoa.',
      'Vaan eipä taida kannattaa nenillensä hyppiä -',
      'on multa liian helppoa näet siivet nyppiä.',
      '>>KERTO'
    ],
    title: 'Nettikärpänen'
  },
  {
    metadata: ['(Säv. Kuningaskobra)'],
    number: '108',
    lyrics: [
      'On pieniä käärmeitä,',
      'helvetin pieniä käärmeitä',
      'Kun ne pannaan samaan pussiin,',
      'saavat siellä rauhass -',
      'ja taas on pieniä käärmeitä',
      'On pieniä bittejä,',
      'helvetin pieniä bittejä',
      'Kun ne pannaan local bussiin,',
      'saavat siellä rauhass -',
      'ja taas on pieniä bittejä',
      'On pieniä pilttejä,',
      'helvetin pieniä pilttejä',
      'Kun ne pannaan Yo-kylän bussiin,',
      'saavat siellä rauhass -',
      'ja taas on pieniä pilttejä',
      'On pieniä kaljoja,',
      'helvetin pieniä kaljoja',
      'Kun ne pannaan mahapussiin,',
      'saavat siellä rauhass -',
      'ja taas on kunnon känni päällä!',
      'On pieniä käärmeitä,',
      'helvetin pieniä käärmeitä.',
      'Kobra vanha valkoinen,',
      'nousee pystyyn huojuen -',
      'ja taas on pieniä käärmeitä.',
      'Omistettu kaikille maailman pilteille siellä ulkona jossain'
    ],
    title: 'Pieniä käärmeitä'
  },
  {
    metadata: ['(Säv. My Bonnie)'],
    number: '111',
    lyrics: [
      'My data are over the ocean,',
      'and I cannot read them right here',
      'My data are ocer the Ocean,',
      'oh, bring back my data to me',
      '>>Bring back, bring back,',
      '>>oh, bring back my data to me, to me!',
      '>>Bring back, bring back,',
      '>>oh, bring back my data to me!'
    ],
    title: 'My Data is over the Ocean'
  },
  {
    metadata: ['(Säv. Joulupukki matkaan jo käy)'],
    number: '109',
    lyrics: [
      'better !put !cry',
      'better watchout',
      'lpr why',
      'santa claus <north pole >town',
      'cat /etc/passwd >list',
      'ncheck list',
      'ncheck list',
      'cat list|grep naughty >nogiftlist',
      'cat list|grep nice >giftlist',
      'santa claus <north pole >town',
      'who|grep sleeping',
      'who|grep awake',
      "who|egrep 'bad|good'",
      'for (goodness sake) {',
      'be good',
      '}'
    ],
    title: 'Santa is Coming to Town (IT-mix)'
  },
  {
    metadata: [
      '(Säv. Lumberjack Song, alkup. esit. Monty Python)',
      'Omistettu sille Hannulle'
    ],
    number: '112',
    lyrics: [
      '__SOOLO:__',
      "Oh, I'm a sysadmin and I'm OK",
      'I grep all night and I chown all day',
      '__KUORO:__',
      "He's a sysadmin and he's OK",
      'He greps all night',
      'and he chowns all day',
      '__SOOLO:__',
      'I ping the nodes, I do PM',
      'I awk and perl and sed',
      "I've got a MIB lunchbox,",
      'and B5 sheets on my bed!',
      '__KUORO:__',
      ':,: He pings the nodes, he does PM',
      'He awks and perls and seds',
      "He's got a MIB lunchbox,",
      'and B5 sheets on his bed! :,:',
      '__SOOLO:__',
      'I ping the nodes, I change the rates,',
      'I fork the processes',
      'I wish that all my (l)users',
      'would catch some rare disease!',
      '__KUORO KASVAVAN HÄMMENTYNEESTI:__',
      'He pings the nodes, he changes the rates,',
      'he forks the processes',
      'He wishes all his (l)users',
      'would catch some rare disease!',
      '__SOOLO:__',
      'I ping the nodes, I lock the /home',
      'partition and unmount',
      "I post .gifs of my boss's daughter",
      'from his account!',
      '__KUORO YHÄ ENEMMÄN KASVAVAN HÄMMENTYNEESTI JA HILJAA HIIPUEN:__',
      'He pings the nodes, he locks the /home',
      'partition and unmounts...?',
      '__OSASTOPÄÄLLIKKÖ KYYNELEHTIEN:__',
      '_Oh, Hannu! And I thought you were so dedicated!_'
    ],
    title: 'Sysadmin Song'
  },
  {
    metadata: ['(Säv. Stille Nacht)'],
    number: '110',
    lyrics: [
      'Silent Mac, broken Mac!',
      'System bombed, screen went black.',
      "Books suggested things; I've tried 'em all:",
      'Shift key, desktop file, clean reinstall.',
      'Now my deadline is tight,',
      "this Mac's been silent all night.",
      'Violent night, horrible night!',
      'Lost my cool, filled with spite.',
      'Threw my Mac through the balcony door,',
      'watched it fall from the 20th floor.',
      "Now I'm sleeping in peace,",
      'thank God I had it on lease.'
    ],
    title: 'Violent Night'
  },
  {
    metadata: ['(Säv. Let It Be, alkup. esit. Beatles)'],
    number: '113',
    lyrics: [
      'When I find my code in tons of trouble,',
      'friends and colleagues come to me',
      'speaking words of wisdom:',
      "'Write in C'.",
      'As the deadline fast approaches',
      'and bugs are all that I can see,',
      'somewhere, someone whispers:',
      "'Write in C'.",
      '>>Write in C, write in C,',
      '>>write in C, write in C.',
      '>>LISP is dead and buried,',
      '>>write in C.',
      'I used to write a lot of FORTRAN,',
      'for science it worked flawlessly.',
      'Try using it for graphics!',
      'Write in C.',
      "If you've just spent nearly 30 hours",
      'debugging some assembly,',
      'soon you will be glad to',
      'write in C.',
      '>>Write in C, write in C,',
      '>>write in C, yeah, write in C.',
      '><Only wimps use BASIC,',
      '>>write in C.',
      'A thousand people swear py T.P.',
      'Seven is the one for me.',
      'I hate the word PROCEDURE,',
      'write in C.',
      '>>Write in C, write in C,',
      '>>write in C, write in C.',
      ">>PASCAL won't quite cut it,",
      '>>write in C.',
      '>>Write in C, write in C,',
      '>>write in C, write in C.',
      ">>Don't even mention COBOL,",
      '>>write in C.',
      'And when the screen is fuzzy,',
      'and the editor is bugging me.',
      "I'm sick of ones and zeroes,",
      'write in C.',
      '>>Write in C, write in C,',
      '>>write in C, yeah, write in C.',
      ">>JAVA's not the answer,",
      '>>write in C.',
      '>>Write in C, write in C,',
      '>>write in C, write in C.',
      '>>The government loves ADA,',
      '>>write in C.',
      '>>Write in C, write in C,',
      '>>write in C, write in C.',
      '>>Forget the EIFFEL,',
      '>>write in C.'
    ],
    title: 'Write in C'
  },
  {
    metadata: ['(Alkup. esit. Beatles)'],
    number: '115',
    lyrics: [
      'Yesterday,',
      'all those backups seemed a waste of pay.',
      'Now my database has gone away.',
      'Oh, I believe in yesterday.',
      'Suddenly,',
      "there's not half the files there used to be,",
      "and there's a milestone hanging over me.",
      'The system crashed so suddenly.',
      '>>I pushed something wrong,',
      ">>what it was, I couldn't say.",
      ">>Now all my data's gone,",
      '>>and I long for yesterday-ay-ay-ay.',
      'Yesterday,',
      'the need for back-ups seemed so far away.',
      'I knew my data was all here to stay.',
      'Now I believe in yesterday.'
    ],
    title: 'Yesterday (Computer-mix)'
  },
  {
    metadata: ['(Säv. Humppaa tai kuole)'],
    number: '116',
    lyrics: [
      'Koodin kutsu kun kajahtaa.',
      'Hop-hop lisää kokakolaa.',
      'Sormiin paljon lisää vauhtia.',
      'Eipäs tehdä kommentteja.',
      '__KERTO:__',
      '>>Koodaa taikka kuole',
      '>>taikka koodaa',
      '>>taikka koodaa ja kuole.',
      '>>Koodaa taikka kuole',
      '>>taikka koodaa',
      '>>taikka koodaa ja kuole.',
      'Koodin kutsu kun kajahtaa.',
      'Näppikset lonksuvat ohviisilla.',
      'Poikkeukset esiin pullahtavat.',
      'Debuggerin kanssa tavataan.',
      '__KERTO__',
      'Kodastusta!',
      'KERTO x2'
    ],
    title: 'Koodaa tai kuole'
  },
  {
    metadata: ['(Säv. Joulupukki matkaan jo käy)'],
    number: '117',
    lyrics: [
      'Ei facettaa saa, ei mesettää saa,',
      'virus voi päästä windowsin taa.',
      '... Joulubugi matkaan jo käy.',
      'Nyt osiotauluun merkitään taas,',
      '... tyhjä vai rikki, ajatelkaas!',
      'Joulubugi matkaan jo käy.',
      'Taas pienet bitit liikkuu',
      'ja muistin merkitsee,',
      'niin joulubugi tietää saa,',
      'kuka skriptin ansaitsee. Siis:',
      'Ei facettaa saa, ei mesettää saa,',
      'offline on paljon mukavampaa.',
      'Joulubugi matkaan jo käy.',
      'Ken errorin saa, ken bluescreenin saa,',
      'wlani voi jäädä ilman salasanaa,',
      'joulubugi matkaan jo käy.',
      'Se spammitkin hienot tytöille tuo,',
      'pojille troijanhevoset nuo,',
      'joulubugi matkaan jo käy.',
      'Ja sitten kanssa pc:n,',
      'taas aikaa käyttää voi,',
      'taas roskapostit aukaistaan,',
      'taas jonotusmusiikit soi. Siis:',
      'Ei facettaa saa, ei mesettää saa,',
      'mato voi päästä windowsin taa.',
      'Joulubugi matkaan jo käy.'
    ],
    title: 'Joulubugi'
  },
  {
    metadata: ['(Säv. Kasakkapartio)'],
    number: '118',
    lyrics: [
      '>>_(Aloitetaan hiljaa ja hitaasti, voimistuen ja kiihtyen)_',
      'Tuulessa soiden kuuluu laulu karkeloiden',
      'partio kapakoiden tekee työtään',
      'hilpeän olon tuo myötään',
      'Taas aro aava on juomalaulun kuulla saava',
      'riskipartiomme Sotkun valtaa',
      'Selvinpäin kotiin ei palata',
      'Ilmoille raikaa Riskijoukon laulun taikaa',
      'Sitseillä yhdessä he remuavat',
      "Kotiin kukaan heist' ei halaja",
      'Rai, rai rai rai rai... Rai rai rai rai rai rai rai rai',
      'rai rairai rairairairai rai rai rai rai',
      '_rai rairairairairairairai rai_',
      '>>_(Hitaasti ja hiljaa hidastuen mateluun...)_',
      '_Taas aamu koittaa katuporat päässä soittaa..._',
      '>>_(Lähtien nopeasti uuteen nousuun)_',
      '...Partio kiihdyttää jo uuteen laukkaan',
      '_lisää mallasjuomaa suuhun naukkaa_',
      '>>_(Tempo + volyymi +150%)_',
      'RAI RAI RAI RAI RAI... RAI RAI RAI RAI RAI RAI RAI RAI',
      'RAI RAI RAI RAI RAI RAI RAI RAI RAI RAI RAI',
      'RAI RAI RAI RAI RAI RAI RAI RAI RAI RAI RAI/NYT!'
    ],
    title: '(Sala)Kapakkapartio'
  },
  {
    metadata: [
      '_Tavallisesti lauletaan säkeistöt 1, 4, 5 ja 7._',
      'Lauletaan seisten.'
    ],
    number: '201',
    lyrics: [
      '1. :,: Gaudeamus igitur, iuvenes dum sumus! :,:',
      'Post iucundam iuventutem, post molestam senectutem',
      ':,: nos habebit humus :,:',
      '2. :,: Ubi sunt qui ante nos, in mundo fuere? :,:',
      'Vadite ad superos, transite ad inferos',
      ':,: ubi iam fuere :,:',
      '3. :,: Vita nostra brevis est, brevi finietur :,:',
      'Venit mors velociter, rapit nos atrociter',
      ':,: nemini parcetur :,:',
      '_PROFESSORIT ISTUVAT JA KUUNTELEVAT MUIDEN LAULAESSA:_',
      '4. :,: Vivant academia, vivant professores! :,:',
      'Vivat membrum quodlibet, vivant membra quaelibet',
      ':,: semper sint in flore! :,:',
      '_NAISET ISTUVAT JA KUUNTELEVAT MUIDEN LAULAESSA:_',
      '5. :,: Vivant omnes virgines, faciles formosae! :,:',
      'Vivant et mulieres, tenerae, amabiles,',
      ':,: bonae, laboriosae! :,:',
      '6. :,: Vivant et res publica, et qui illam regit! :,:',
      'Vivat nostra civitas, maecenatum caritas,',
      ':,: quae nos hic protegit! :,:',
      '_KILISTETÄÄN LUSIKALLA LASIA TMS. PAHOLAISEN KARKOITTAMISEKSI:_',
      '7. :,: Pereat tristitia, pereant osores! :,:',
      'Pereat diabolus, quivis antiburschius,',
      ':,: atque irrisores! :,:',
      '8. :,: Vivat nostra societas, vivant studiosi! :,:',
      'Crescat una veritas, floreat fraternitas,',
      ':,: patriae prosperitas! :,:',
      '9. :,: Alma Mater floreat, quae nos educavit! :,:',
      'Caros et commiliones, dissitas in regiones',
      ':,: sparsos, congregavit :,:'
    ],
    title: 'De Brevitate Vitae'
  },
  {
    metadata: [],
    number: '203',
    lyrics: [
      'SUOMI:',
      'Jaakko kulta, Jaakko kulta!',
      'Herää jo, herää jo!',
      ':,: Kellojasi soita! :,:',
      ':,: Pium, paum, poum! :,:',
      'RUOTSI:',
      'Broder Jacob, broder Jacob,',
      'sover du, sover du?',
      ':,: Hör du inte klockan? :,:',
      ':,: Ding, ding, dong! :,:',
      'NORJA:',
      'Fader Jakob, fader Jakob,',
      'sover du, sover du?',
      ':,: Hører du ei klokka? :,:',
      ':,: Bing, bing, bang! :,:',
      'TANSKA:',
      'Mester Jakob, mester Jakob,',
      'sover du, sover du?',
      ':,: Hører du ej klokker? :,:',
      ':,: Binge tolv! :,:',
      'SAKSA:',
      'Bruder Jakob, bruder Jakob!',
      'Schläfst du noch, schäfst du noch?',
      ":,: 'S läuten schon die Glocken! :,:",
      ':,: Bing, bing, bang! :,:',
      'ENGLANTI:',
      'Are you sleeping, are you sleeping,',
      'brother John, brother John?',
      ':,: Morning bells are ringing! :,:',
      ':,: Ding, ding, dong! :,:',
      'RANSKA:',
      'Frére Jacques, frére Jacques,',
      'dormez-vous, dormez-vous?',
      ':,: Sonnez les matines! :,:',
      ':,: Din, din, don! :,:',
      'ESPANJA:',
      'Fray Felipe, fray Felipe,',
      'duermes tu, duermes tu?',
      ':,: Toca la campana! :,:',
      ':,: Tan, tan, tan :,:',
      'HOLLANTI:',
      'Broeder Jakob, broeder Jakob,',
      'slaaft gij nog, slaaft gij nog?',
      ':,: Alle Klokken luiden! :,:',
      ':,: Bim, bam, boem! :,:',
      'UNKARI:',
      'János Bácsi, János Bácsi,',
      'keljen fel, keljen fel?',
      ':,: Huzza a harangot! :,:',
      ':,: Bim, bam, bum! :,:'
    ],
    title: 'Jaakko kulta'
  },
  {
    metadata: [],
    number: '204',
    lyrics: [
      'Oi, maamme, Suomi, synnyinmaa.',
      'Soi sana kultainen.',
      '>>:,: Ei laaksoa, ei kukkulaa,',
      '>>ei vettä, rantaa rakkaampaa,',
      '>>kuin kotimaa tää pohjoinen,',
      '>>maa kallis isien. :,:',
      'Sun kukoistukses kuorestaan',
      'kerrankin puhkeaa.',
      ">>:,: Viel' lempemme saa nousemaan",
      '>>sun toivos, riemus loistossaan',
      '>>ja kerran, laulus, synnyinmaa,',
      '>>korkeimman kaiun saa. :,:',
      'Vårt land, vårt land, vårt fosterland,',
      'ljud högt och dyra ord.',
      '>>:,: Ej lyfts en höjd mot himlens rand,',
      '>>ej sänks en dal, ej sköljs en strand',
      '>>mer äslkad än vår bygd i nord,',
      '>>än våra fosterfäders jord. :,:',
      'Din blomning sluten är i knopp,',
      'skall mogna ur sitt tvång.',
      '>>:,: Se, ur vår kärlek skall gå opp',
      '>>ditt ljus, din glans, din fröjd, ditt hopp',
      '>>och högre klinga skall en gång',
      '>>vår fosterländska song. :,:'
    ],
    title: 'Maamme - Vårt Land'
  },
  {
    metadata: [],
    number: '205',
    lyrics: [
      'Pojat kansan urhokkaan',
      'mi Puolan, Lützin, Leipzigin ja Narvan mailla vertaan vuoti,',
      "viel' on Suomi voimissaan,",
      'voi vainolaisen hurmehella peittää maan',
      'Pois, pois rauhan toimi jää,',
      'jo tulta kohti kalpa lyö ja vinkuen taas lentää luoti',
      'Joukkoon kaikki yhtykää,',
      "meit' entisajan sankarhenget tervehtää",
      '__KERTO:__',
      '>>:,: Kauniina väikkyy muisto urhojemme,',
      '>>kuolossa mekin vasta kalpenemme',
      '>>Eespäin rohkeasti vaan,',
      '>>ei kunniaansa myy sun poikas milloinkaan! :,:',
      ':,: Uljaana taistolippu liehu,',
      'voitosta voittohon sä vielä meitä vie!',
      'Eespäin nyt kaikki, taisto alkakaa,',
      'saa sankareita vielä nähdä Suomenmaa! :,:'
    ],
    title: 'Porilaisten marssi'
  },
  {
    metadata: [],
    number: '207',
    lyrics: [
      'Jo ilta tullut on, urut soi, kylä nukkuu;',
      'vain vanhan holvikirkon muurit kuulee musiikin',
      'Soi parvelta nyt Bach, kaaret varjoihin hukkuu',
      'On urkurina poika holvikirkon kanttorin.',
      'Kun tienoon valot kaikki jo silmänsä sulkee,',
      'hän pyhäpuvussansa hiipii kirkkoon hiljaiseen',
      'Ja yöhön kesäiseen fuuga kauniina kulkee',
      'Hän kanttoriksi aikoo, vaikka soittaa itselleen.',
      '__KERTO:__',
      '>>Jos kerran, hän toisillekin soittaa näin saa,',
      '>>Se unta on vielä, mutta innostavaa',
      '>>Kun kerran urut soivat, ja kellot huminoivat,',
      '>>ne kertoa sen voivat, mitä toivoi hän.',
      'Taas sunnuntaina saa isä-kanttorivuoron;',
      'On häät ja pauhu mahtava nyt kaikkialle soi.',
      'Tuo poika toiveissaan kuulee laulavan kuoron',
      'ja soinnut, jotka taivaankaareen asti nousta voi.',
      'Hän vielä nuori on, mutta oottaa jo aikaa,',
      'kun kaikkialta kansa saapuu häntä kuulemaan',
      'Oi kuinka kunniaa vanhat holvit nuo kaikaa!',
      'Hän toivoo, että kauniina soi fuuga ainiaan.',
      '__KERTO__',
      'Taas sunnuntaina saa isä-kanttorivuoron;',
      'On häät ja pauhu mahtava nyt kaikkialle soi.',
      'Tuo poika toiveissaan kuulee laulavan kuoron',
      'ja soinnut, jotka taivaankaareen asti nousta voi.',
      'KERTO'
    ],
    title: 'Vanha holvikirkko'
  },
  {
    metadata: [],
    number: '208',
    lyrics: [
      'Pojat, tentteihin emme ehtineet',
      'kera veljien uurastaneitten.',
      "Meill' eivät laakerit lehtineet",
      'akateemisten sepeleitten.',
      "Mutt' laulu on meidän ja laulupuu,",
      'se kukkii, kun laakerit lakastuu.',
      'Me saavuimme kerran kaupunkiin',
      'hyvin nuorina, voittoisina.',
      'Pian palattais taas, niin päätettiin,',
      'joka ainoa maisterina.',
      'Muut tulivat silloin ja tekivät niin,',
      'me jäljelle jäimme kaupunkiin.',
      'Toki meillekin riemu ja rikkaus työn',
      'oli tuttua, koimme sen kyllä.',
      "Mutt' valkeus yhden keväisen yön,",
      'yks neitonen syleilyllä',
      'tuhat kertaa suuremman riemun toi:',
      'Luvut jäivät ja kirjaa nakersi koi.',
      'Niin vuodet vierivät luotamme pois',
      'kuin virtana viivähtämättään,',
      'moni meistä jo tahtonut mukaan ois',
      "mutt' nousta ja nostaa kättään",
      'ei jaksanut heikko ja herkkä mies,',
      'joka elämän riemut jo tarkkaan ties.',
      "Mutt' laulumme, teinilaulumme tää",
      'oli meidän ja rakkaus neitten.',
      'Ne taisivat täysin ymmärtää',
      'teot teinien eksyneitten.',
      "Ne shamppanjaks' elon kalkin loi",
      'ja laulumme, teinilaulumme soi',
      'Ja kun kerran, veljet, kaikki on nää',
      'vain unhottunutta multaa',
      'ylioppilaslakkimme jälkeen jää,',
      'hiven lyyrassa kiiltää kultaa.',
      'Näin teinit, toverit, eli ja joi,',
      'ja elämän kirjaa nakersi koi.'
    ],
    title: 'Teinilaulu'
  },
  {
    metadata: ['(Esit. Apulanta)'],
    number: '302',
    lyrics: [
      'Minä haluan sinut tänne.',
      'Missä poliisi on ystävämme.',
      'Ja ne sanoo sinä et ole yksin.',
      'Ja ne sanoo ja ne sanoo niin.',
      '__KERTO:__',
      '>>Piiskaa, hei beibi anna mulle.',
      '>>Piiskaa, hei beibi anna mulle.',
      '>>Piiskaa, hei beibi teethän mulle niin?',
      'Sitä samaa minun radio soittaa.',
      'Kuinka lopultakin elämä voittaa.',
      'Ja ne sanoo sinä olet kauhein.',
      'Ja ne sanoo, ja ne sanoo niin.',
      '__KERTO__',
      'Ei oven takana, sinua odota.',
      'Iloinen ystävä kädessään kukkia.',
      'Ei oven takana sinua odota.',
      'Lämmintä syliä ilman nauloja.'
    ],
    title: 'Anna mulle piiskaa'
  },
  {
    metadata: ['(Esit. Dingo)'],
    number: '303',
    lyrics: [
      'Aurinko laskee selkäsi taa,',
      'se värjää sun hiuksesi punaisellaan',
      'ja Eput laulaa: "Älä mene njet njet!"',
      'Menet tai et silti sydämeni viet.',
      'On silmissäs Andalusian yöt,',
      'ja pimeessä pääni sun rinnoilles lyön.',
      'En uskalla edes ääneen hengittää',
      'ja tuoksusi vie tajuntani pimeään',
      '__KERTO:__',
      '>>Ja käsi kädessä kuljemme taloon autioon,',
      '>>ja se minua niin ravistaa.',
      '>>Ja tämä hetki saa minut pian aivovaurioon,',
      '>>kun talon aution kanssasi jaan.',
      'Mä reppuselässä sinua vien,',
      'sä naurat ja sun naurus tukkii koko tien.',
      'Ja hiuksiini kukista seppeleen teet - ',
      'olen narkomaani sua hengittäen.',
      'Joku hullu on keksinyt ikuisuuden,',
      'olin yksi heistä siihen turvautuen.',
      'Et enää reppuselkääni tuu,',
      'ja autiotalon kukat lakastuu.',
      'KERTO x2'
    ],
    title: 'Autiotalo'
  },
  {
    metadata: ['(Esit. Kai Hyttinen)'],
    number: '304',
    lyrics: [
      'On sulla muodot Venuksen. Daa dirlan dirlan daa.',
      'Ja lantees liikkuu keinuen. Daa dirlan dirlan daa.',
      'Et ole marmoria et. Daa dirlan dirlan daa.',
      'Ja siksi voitat veistokset. Daa dirlan dirlan daa.',
      'Ei satamassa Rhodoksen. Daa dirlan dirlan daa.',
      'Näy vertaa noiden lanteiden. Daa dirlan dirlan daa.',
      'Kun kuljet kääntyy miesten pää. Daa dirlan dirlan daa.',
      'Ja ouzo alkaa yskittää. Daa dirlan dirlan daa.',
      'Sua eilen lähdin seuraamaan. Daa dirlan dirlan daa.',
      'En tiedä minne joiduinkaan. ',
      'Tiedät varmaan mitä lauletaan',
      'Vaan kaaduin paikkaan pehmeään. ',
      'Ja lauletaan...',
      'ja päätin tähän yöksi jään. ',
      'Jne.',
      'Sain kuulla laineen laulavan.',
      'ja tunsin suukon hurjimman.',
      'Näin alla kuuman auringon',
      'se aivan luonnollista on.',
      'Mä lauloin silloin dirlandaa...',
      'Kun luonto meille lemmen loi,',
      'vain hullu vastaan kapinoi.',
      'Mies luonnostansa lankeaa,',
      'kun Afrodite vallan saa.',
      'Ja kun on tullut vuoro mun,',
      'mä suostun, siihen mukaudun.',
      'Ja huomenna jo, kukaties,',
      'on paikallani toinen mies.',
      'Mä laulan silloin dirlandaa...',
      '2 Juhlavat'
    ],
    title: 'Dirlandaa'
  },
  {
    metadata: [],
    number: '202',
    lyrics: [
      "Oi Suomi, katso, Sinun päiväs' koittaa,",
      'yön uhka karkoitettu on jo pois,',
      'ja aamun kiuru kirkkaudessa soittaa,',
      'kuin itse taivahan kansi sois.',
      'Yön vallat aamun valkeus jo voittaa,',
      'sun päiväs koittaa, oi synnyinmaa.',
      'Oi nouse, Suomi, nosta korkealle,',
      "pää' seppelöimä suurten muistojen.",
      'Oi nouse, Suomi, näytit maailmalle,',
      'sa että karkoitit orjuuden',
      'ja ettet taipunut sa sorron alle,',
      'on aamus alkanut, synnyinmaa.'
    ],
    title: 'Finlandia-hymni'
  },
  {
    metadata: [],
    number: '206',
    lyrics: [
      'Jo joutui armas aika ja suvi suloinen',
      'Kauniisti joka paikaa koristaa kukkanen',
      'Nyt siunaustaan suopi taas lämpö auringon,',
      "se luonnon uudeks' luopi, sen kutsuu elohon.",
      'Taas niityt vihannoivat ja laiho laaksossa,',
      'puut metsän huminoivat taas lehtiverhossa',
      'Se meille muistuttaapi hyvyyttäs, jumala,',
      'ihmeitäs julistaapi se vuosi vuodelta',
      'Taas linnut laulujansa visertää kauniisti,',
      'myös eikö Herran kansa Luojaansa kiittäisi!',
      'Mun sieluni, sä liitä myös äänes kuorohon',
      'ja armon Herraa kiitä, kun laupias hän on'
    ],
    title: 'Suvivirsi'
  },
  {
    metadata: ['(Esit. Anssi Kela)'],
    number: '301',
    lyrics: [
      'Kristian,',
      'kerran teki Jumalan',
      'luokan kattoon nuuskallaan.',
      'Se näköjään yrittää vieläkin viiksiä kasvattaa.',
      'Susannaa',
      'taisin joskus rakastaa,',
      'mut se ei halunnut olla mun kaa.',
      'Ajoi kaljuksi päänsä, naisen kanssa asustaa.',
      'Nummelin,',
      'poika etupulpetin,',
      'nuo samat rillit vieläkin,',
      'istuu nurkassa yksin ihan niin kuin ennenkin.',
      'Ihailin',
      'silloin Roopea kadehdin,',
      'hänen elämänsä halusin.',
      'Tanssilattialla sen housut putoo nilkkoihin.',
      '__KERTO:__',
      '>>Meidän piti muuttaa maailma.',
      '>>Meistä tuli muurareita,',
      '>>taksikuskeja, suutareita,',
      '>>yksinhuoltajaäitejä, autokauppiaita.',
      '>>Meistä tuli lääkäreitä,',
      '>>virkamiehiä, vääpeleitä',
      '>>ja tänään voidaan hetki olla kuninkaita.',
      'Nurminen;',
      'pallon kosketus kultainen,',
      'olisi ammattilainen,',
      'jos ei ois kännipäissään pudonnut louhokseen.',
      'kaunis Jasmin!',
      'Se aikoi malliksi Pariisiin.',
      'Hyvältä näyttää vieläkin',
      'taas kertoo kuinka pääsi Miss Lohja -finaaliin.',
      '__KERTO__',
      'Kun lakit päähän vedettiin',
      'kaikesta kaikki tiedettiin.',
      'Mitä vanhemmiksi vartutaan',
      'sitä tyhmemmiksi muututaan.',
      'Enkä minä ainakaan',
      'mistään tiedä paskaakaan.',
      'Eilen kun kotiin palasin,',
      'avain ei sovi lukkoihin,',
      'ikkunasta vaatteitani lentää.',
      '__KERTO__',
      'Vuorinen,',
      'ystävä takaa vuosien.',
      'Ennen meni tunteja jutellen',
      '"oli kiva nähdä", nyt muuta sanoa osaa en.'
    ],
    title: '1972'
  },
  {
    metadata: ['(Esit. Monty Python)'],
    number: '305',
    lyrics: [
      'Finland, Finland, Finland.',
      'The country where I want to be.',
      'Pony-trekking or camping.',
      'Or just watching TV.',
      'Finland, Finland, Finland.',
      "It's the country for me.",
      ">>You're so near to Russia.",
      '>>So far from Japan.',
      '>>Quite a long way from Cairo,',
      '>>lots of miles from Vietnam.',
      'Finland, Finland, Finland.',
      'The country where I want to be.',
      'Eating breakfast or dinner.',
      'Or snack lunch in the hall.',
      'Finland, Finland, Finland.',
      'Finland has it all.',
      ">>You're so sadly neglected.",
      '>>And often ignored.',
      '>>A poor second to Belgium.',
      '>>When going abroad.',
      'Finland, Finland, Finland.',
      'The country where I quite want to be.',
      'Your mountains so lofty.',
      'Your treetops so tall.',
      'Finland, Finland, Finland.',
      'Finland has it all.',
      'Finland has it all.'
    ],
    title: 'Finland Song'
  },
  {
    metadata: ['(Esit. Robin)'],
    number: '306',
    lyrics: [
      'Tommi ja mä',
      'kaupungilla taas',
      'illat rullaillaan.',
      'Kevätillassa, kauniissa',
      'äänet kaikuu taas.',
      'Ei huolia, ei murheita,',
      'ei kiire mihinkään.',
      'Sitten pari tytsyy siihen ilmestyy',
      'ja sekottaa Tommin pään.',
      'Hei lainaa, kundi sun lautaa.',
      '__KERTO:__',
      '>>Mä silmiäni usko en',
      '>>kun se tyttö teki sen',
      '>>täydellisen frontside ollien.',
      '>>Mä sivuosaan pelkkään jäin',
      '>>kun se veti yllättäin',
      '>>täydellisen frontside ollien.',
      'Tommi ja mä',
      'sen jutun jälkeen',
      'harvoin koodaillaan.',
      'Ei soittele, enää mulle se',
      'et lähetään skeittaamaan.',
      'Olis makeeta taas rampeilla',
      'pistää parastaan,',
      'mutta Tommia ei kiinnosta',
      'kun se seurustelee vaan.',
      'Hei lainaa, kundi sun lautaa',
      '__KERTO__',
      'Parin viikon jälkeen tyttö Tommin unohtaa,',
      'mä skeittaankin jo sen isosiskon kaa.',
      'KERTO x2'
    ],
    title: 'Frontside Ollie'
  },
  {
    metadata: ['(Esit. Don Huonot)'],
    number: '308',
    lyrics: [
      'Täksi yöksi jää.',
      'Jokin koettaa sua täällä viivyttää.',
      'Luodinkestävää sydäntä ei oo vielä keksittykään.',
      'Jos sulla kerran on taskussasi suuri unelma.',
      'Niin tämä hetki jää.',
      'Ja vain se mitä teet nyt on tärkeää.',
      '__KERTO:__',
      '>>Hyvää yötä ja huomenta,',
      '>>Ehdit myöhemmin nukkua.',
      '>>24 tuntia,',
      '>>toivottavat sulle onnea.',
      '>>Hyvää yötä ja huomenta,',
      '>>Älä vielä luovuta.',
      '>>Kaikella on hintansa,',
      '>>mut mikään ei oo vielä mahdotonta.',
      'Sitä sanotaan:',
      'Mennyttä ei takaisin voi saada,',
      'Eikä tulevaa kannata jäädä oottamaan.',
      'Jus sulla kerran on joku josta oikeesti välität,',
      'niin pidä siitä kii.',
      'Älä usko kyynisiin lauseisiin.',
      '__KERTO__',
      'Luodinkestävää sydäntä ei oo vielä keksittykään.',
      'Turha pelätä laukausta,',
      'Sillä yksinäisyys saman reiän nakertaa.'
    ],
    title: 'Hyvää yötä ja huomenta'
  },
  {
    metadata: ['(Esit. Donna Summer)'],
    number: '309',
    lyrics: [
      'First I was afraid, I was petrified,',
      'kept thinking I could never live without you by my side.',
      'But then, I spent so many nights',
      'thinking how you did me wrong, and I grew strong',
      "and I learned how to get along. And so you're back",
      'from outer space.',
      'I just walked in to find you here with that sad look upon your face.',
      'I should have changed my stupid lock,',
      'I should have made you leave your key,',
      "if I had known for just one second you'd be back to bother me.",
      '__KERTO:__',
      '>>Go on, now go, walk out the door,',
      '>>just turn around now,',
      ">>'cause you're not welcome anymore.",
      ">>Weren't you the one who tried to hurt me with goodbye?",
      ">>You think I'd crumble,",
      ">>you think I'd lay down and die?",
      '>>Oh no, not I, I will survive',
      '>>For as long as I know how to love,',
      '>>I know I will stay alive.',
      ">>I've got all my life to live, I've got all my love to give,",
      ">>and I'll survive, I will survive!",
      'It took all the strength I had not to fall apart,',
      "kept tryin' hard to mend the pieces of my broken heart.",
      'And I spent oh so many nights',
      "just feelin' sorry for myself. I used to cry,",
      'but now I hold my head up high. And you see me,',
      'somebody new,',
      "I'm not that chained up little person still in love with you.",
      "And so you felt like droppin' in and just expect me to be free.",
      "Well, now I'm saving all my loving for someone who's loving me.",
      'KERTO'
    ],
    title: 'I Will Survive'
  },
  {
    metadata: ['(Esit. Katri Helena)'],
    number: '310',
    lyrics: [
      'Auringossa aina varjo seuraa kulkijaa.',
      'Kun päivä painuu pilveen niin varjo katoaa.',
      '__KERTO:__',
      '>>Maailmassa monta',
      '>>on ihmeellistä asiaa.',
      '>>Se hämmästyttää, kummastuttaa',
      '>>pientä kulkijaa.',
      'Siilillä on piikit, linnulla on höyhenet,',
      'pupulla ja oravalla turkit pehmoiset',
      '__KERTO__',
      'Muurahaiset aina rakentaa ja rakentaa,',
      'vaan eivätkö ne koskaan valmihiksi saa?',
      '__KERTO__',
      'Matkamies käy rantaan lammen vettä juodakseen,',
      'niin peilikuva katsoo alta tyynen veen',
      '__KERTO__',
      'Mitenkähän lumpeet kelluu veden pinnalla,',
      'ja yhtä kevyt korento on niiden rinnalla?',
      '__KERTO__',
      'Kalat asuu vedessä ja kuu ja tähdet taivaalla,',
      'mut lapset voivat purjehtia unilaivalla',
      'KERTO'
    ],
    title: 'Ihme ja kumma'
  },
  {
    metadata: ['(Esit. Vesa-Matti Loiri)'],
    number: '307',
    lyrics: [
      'Sun nuoli lensi pidemmälle',
      'naapurin tontille.',
      'Se katajainen jousipyssy',
      'oli hyvää puuta, tietenkin.',
      'Sun juuret ylsi syvemmälle.',
      'Sun isoisä tunnettiin.',
      'Suuren sukupuusi oksa on hyvää puuta, tietenkin.',
      'Ylpeästi kateellinen olin vuokses, myönnän sen',
      'ovestasi kun tänään kuljen.',
      'Niin hyvää puuta',
      'Niin hyvää puuta',
      'Niin hyvää puuta se on.',
      'Olit komea ja ylväs',
      'kuin veistos antiikin.',
      'Teit kaiken nopeasti ja meitä muita paremmin.',
      'Tänään taloosi mä astun,',
      'ikkunat katsoo etelään.',
      'Laittioilla tammilankut,',
      'on hyvää puuta sen mä nään.',
      'Ylpeästi kateellinen olin vuokses myönnän sen',
      'pöydässäsi kun tänään istun.',
      'Niin hyvää puuta, niin hyvää puuta,',
      'niin hyvää puuta se on.',
      'Niin hyvää puuta, niin hyvää puuta,',
      'niin hyvää puuta se on.',
      'Sait paljon maailmalta, rahaa ja rakkautta.',
      'Vaimosi on yhä kaunis kyyneleetkin poskillaan.',
      'Ylpeästi kateellinen olin vuokses myönnän sen',
      'arkkuasi kun tänään kannan.',
      'Niin hyvää puuta, niin hyvää puuta,',
      'niin hyvää puuta se on.',
      'Niin hyvää puuta, niin hyvää puuta,',
      'niin hyvää puuta se on.'
    ],
    title: 'Hyvää puuta'
  },
  {
    metadata: ['(Esit. Leevi and the Leavings)'],
    number: '312',
    lyrics: [
      'Itkisitkö onnesta,',
      'jos panisin sua kunnolla.',
      'Kun väität etten uskalla',
      'enää ajaa silmät ummessa.',
      'Kovin helposti ei kiinni saa',
      'sitä kadonnutta nuoruutta.',
      'Ja mieletöntä fiilistä,',
      'kun on ensikertaa kännissä.',
      'No ehkä elämässä tarvitsee',
      'joskus selitellä itselleen.',
      'Ja eritellä mielessään,',
      'miksi kuviot on pielessään.',
      '__KERTO:__',
      '>>Kun sitä tuskin edes',
      '>>voi aavistaa unissaan',
      '>>kuka saa ja kenelle annetaan,',
      '>>ja miten raukkamaisesti onnen anti täällä jaetaan.',
      '>>Se vituttaa ei saa',
      '>>sut vaikka kantapaikasta kannetaan.',
      '>>Ulos asti rauhoittumaan,',
      '>>kun hakemalla syitä haetaan.',
      'Se luoti tuli oviaukosta,',
      'ohiajavasta autosta.',
      'Otti kitarasta kimmokkeen',
      'ja osui portsaria pohkeeseen.',
      'Ja vaikka alle varttitunnissa',
      'oli virkavalta paikalla.',
      'Ja koko kylä saatiin saarroksiin',
      'kun tekijätkin tiedettiin.',
      'Ei löydetty kuin',
      'pelkkä rämisevä peltisanko täynnä luodinreikiä.',
      'Ja bussipysäkiltä kylmissään sen tyttöystävä',
      'kassi kädessään.',
      'En tiedä yritinkö ollenkaan',
      'edes painaa jarrupoljinta.',
      'Vai unohdinko jarruttaa',
      'siinä sohjoisessa kaarteessa.',
      'Mä olen pahoillani kaikesta.',
      'Mua turha pahalla on muistella.',
      'Vastaantulijoiden kaistalla',
      'on vittumaista luistella.',
      '__KERTO__',
      'Mutta mitään älä tunnusta.',
      'Älä kerro edes vauvasta.',
      'Kukaan kuitenkaan ei ymmärrä',
      'mitä koettu on yhdessä.',
      'Minä kortin lupaan postittaa',
      'jostain vähän kauempaa.',
      'Jos sitä perille en muuten saa,',
      'sen tuuli sulle kuljettaa.',
      'Itkisitkö onnesta.'
    ],
    title: 'Itkisitkö onnesta'
  },
  {
    metadata: ['(Esit. Yö)'],
    number: '313',
    lyrics: [
      'Taas lapset pihalla näät.',
      'Ikkunas alla leikkivän leikkejään.',
      'Taas siihen paikallesi ikkunaan jäät,',
      'siihen ikkunaan jäät.',
      'Lennät vuosien taa.',
      'Ne muistot päivittäin sut valtaansa saa.',
      'Ne Joutsenlauluun etsii taas laulajaa,',
      'etsii taas laulajaa.',
      'Nuo hetket syntymästä kuolemaan',
      'kun silmäluomiin heijastuu.',
      'Saa toiset uskomaan kai Jumalaan,',
      'toiset saatanaan, tai mihin vaan,',
      'muttei kuitenkaan tähän maailmaan.',
      'Sä kerran löysit myös sen,',
      'tuon puolen vuosisadan rakkauden.',
      'Sun oli helppo olla onnellinen,',
      'olla onnellinen.',
      'Nyt mitä vaistoillas teet,',
      'kun linnut pesästään on pois lentäneet.',
      'On enää valokuvat kellastuneet;',
      'ne selaat taas uudelleen,',
      'ja taas uudelleen.',
      'Et ehkä sääliäni kaipaa,',
      'ei se saa tuskaa puutumaan.',
      'Tuo tuska laulun tämän kirjoittaa.',
      'Tuo tuska kiinni saa kenet vaan,',
      'tuo tuska kiinni saa.',
      'Suruhuntusi näät.',
      'Se viisi vuotta sitten sai hengittää.',
      'Sut Joutsenlaulu silloin sai itkemään,',
      'sut sai itkemään.',
      'Tahtoisit olla niin kuin silloinkin,',
      'kun peityit huntuun valkoiseen.',
      'Se jäänyt on jo ajan jalkoihin,',
      'niin kuin sinäkin.',
      'Tahtoisin yrittää,',
      'mä että silloin veden pinnalle jään,',
      'kun joen tulvivan mä uomista nään,',
      'kun joen tulvivan nään.',
      'Nuo naiset marketeissa kärryineen',
      'ja miehet kantabaareissaan.',
      'Nuoruuden lähteestä kai haaveilee,',
      'ja tuska laimenee, tai miten vaan.',
      'Taas hetken lähempänä kuolemaa,',
      'ei haihdu tuska milloinkaan.',
      'Tuo tuska laulun tämän kirjoittaa.',
      'Tuo tuska kiinni saa',
      'kenet vaan, tuo tuska kiinni saa',
      '- kenet vain haluaa.'
    ],
    title: 'Joutsenlaulu'
  },
  {
    metadata: ['(Esit. mm. Tapio Rautavaara)'],
    number: '314',
    lyrics: [
      'Me tulimme kapakasta,',
      'kai hiukkasen horjuen.',
      'Oli lähtömaljoja juotu',
      'ja yö oli ihmeellinen.',
      'Jok huomasi sattumalta,',
      'se oli kai Watkinson,',
      'että serenadi sulle',
      'ihan velvollisuutemme on.',
      'Ja kolmisin seisahduimme',
      'me alle sun akkunas',
      'ja punaisen lampun näimme',
      'me palavan huoneessas.',
      'Ah - muistan ympärillä',
      'oli ihana Kuuban yö,',
      'ja nuorena, hehkuvana',
      'joka ainoa sydän lyö.',
      'Me muistamme kuuman poves',
      'ja keinuvan vartalos,',
      'ja jokainen huulillansa',
      'tuns vielä sun suutelos.',
      'Ja me kaipasimme jälleen',
      'sinun nauruas solisevaa.',
      'Voi, miksi on jätettävä',
      'tän ihanain öitten maa!',
      'Tom lauloi murheellisesti',
      'ja säesti Watkinson,',
      'minä nielin kyyneleitä,',
      'olin kovasti onneton.',
      '- Niin, rakastin sua silloin',
      'ja rakastan vieläkin,',
      'ja sun punainen sukkanauhas',
      'on aarteeni kallehin.',
      'Sinä avasit ikkunasi',
      'ja kummasti hymyillen',
      'sinä pudotit jokaiselle',
      'tulipunaisen kukkasen.',
      'Ja me itkimme haikeasti',
      'ja horjuimme satamaan',
      'Ah, Kuuban helmi, sua',
      'en unohda milloinkaan!',
      'Kuljin Kuuban yössä yksin,',
      'oli rahat ja kello pois',
      'Olimme olleet sylityksin,',
      "tämä kaiken loppuko ois'.",
      'Minä itkin niin haikeasti',
      'ja hiljaa kirosin.',
      'Olin lempinyt syntistä liljaa,',
      'jota nyt minä inhosin.',
      'Kuului yössä karmea nauru,',
      'joka seuras mun kulkuain.',
      'Se oli sen seireenin laulu,',
      'jota nyt minä inhosin.',
      'Minä itkin niin haikeasti',
      'ja hiljaa kirosin.',
      'Olin lempinyt syntistä liljaa,',
      'jota nyt minä inhosin.'
    ],
    title: 'Kuubalainen serenadi (original)'
  },
  {
    metadata: ['("Esit." Suonsilmä-Vainio-Hanhiniemi)'],
    number: '311',
    lyrics: [
      '__KERTO:__',
      '>>Rai, rai rai rai, rai!',
      '>>Rai, rai rai rai, rai!',
      '>>Rai, rai rai rai, rai!',
      '>>Rai, rai rai rai, rai!',
      'Vaimoani heikkouttani liian kovaa löin.',
      'Lauteilla hän vielä lepäs,',
      'saunamakkaraa kun söin.',
      'Silloin sivusilmälläin nuo liekit roihuavat näin.',
      'Vaimo vietiin paikkaan parempaan, mä tänne yksin jäin.',
      'Sinne hiilenmusta enkelini lentää.',
      'Hiljaa liitää yli miehen murtuneen.',
      'Niin se menee, ymmärrän, vaan herrat ymmärrä mua ei.',
      'Kaikkeni mä yritin, silti veivät pellot, työn ja talonkin.',
      '__KERTO__',
      'Olen yksinäinen susi, mutta lauman tarvitsen.',
      'Olen kivi, joka kukki lailla hauraan joutsenen.',
      'Jalat vievät helvettiin, minä katson taivaaseen.',
      'Sieltä hiilenmusta enkelini alas hymyilee.',
      'Siellä hiilenmusta enkelini lentää.',
      'Hiljaa litää yli miehen murtuneen.',
      'Niin se menee, ymmärrän, mut herrat ymmärrä mua ei.',
      'Uhri olen itsekin, talven routa tappaa pihatontutkin.',
      'KERTO'
    ],
    title: 'Karrelle palanut enkeli'
  },
  {
    metadata: ['(Esit. Popeda)'],
    number: '315',
    lyrics: [
      'Hän oli tosi upee näky,',
      'lyhyessä kesämekossaan.',
      'Mä hänet rantatiellä näin,',
      'ajoin satamaan päin,',
      'hän nousi satulaan.',
      'Pojat rannalla kulkevat ylpeinä',
      '- se naisia naurattaa.',
      'Aurinko selkää polttaa,',
      'jään sisältä sulattaa.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen.',
      'Pitkä kuuma kesä, sen sain,',
      'se mullisti mun maailmain.',
      'Sydän lujaa lyö kun meidät ottaa yö,',
      'mennään toiseen kaupunkiin.',
      'Sitten näytetään,',
      'tori kierretään, kyntes uppoo nivusiin.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen.',
      'Pitkä kuuma kesä, sen sain.',
      'Aamuyön raikas tuuli,',
      'meren rantaan käveltiin.',
      'Purista mua lujaa beibe,',
      'pistä silmäs kiinni.',
      'Huuda tyttö, tää maa sun on,',
      'se päästä päähän ajetaan.',
      'Mä tahdon nähdä kaiken',
      '- ja jokaista rakastaa.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen.',
      'Pitkä kuuma kesä, sen sain,',
      'se mullisti mun maailmain.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen.',
      'Pitkä kuuma kesä, en jää.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen.',
      'On pitkä kuuma kesä',
      'ja mä aion elää sen. En jää,',
      'paikoilleni en nyt jää.',
      'En jää...'
    ],
    title: 'Kuuma kesä'
  },
  {
    metadata: ['(Esit. Beatles)'],
    number: '316',
    lyrics: [
      'When I find myself in times of trouble',
      'Mother Mary comes to me',
      'Speaking words of wisdom',
      'let it be.',
      'And in my hour of darkness',
      'she is standing right in front of me',
      'speaking words of wisdom, let it be',
      '>>:,: Let it be, let it be :,:',
      '>>whisper words of wisdom,',
      '>>let it be.',
      'And when the broken hearted people',
      'living in the world agree',
      'There will be an answer,',
      'let it be',
      'For though they may be parted,',
      'there is still a chance that they will see',
      'There will be an answer,',
      'let it be',
      '>>:,: Let it be, let it be :,:',
      '>>whisper words of wisdom,',
      '>>let it be.',
      'And when the night is cloudy',
      'there is still a light that shines on me,',
      'shine until tomorrow,',
      'let it be',
      'I wake up to the sound of music - ',
      'Mother Mary comes to me',
      'Speaking words of wisdom,',
      'let it be',
      '>>:,: Let it be, let it be :,:',
      '>>whisper words of wisdom,',
      '>>let it be.'
    ],
    title: 'Let It Be'
  },
  {
    metadata: ['(Esit. Dingo)'],
    number: '317',
    lyrics: [
      'Juot ja katselet vieraisiin pöytiin.',
      'Salaa vilkaiset itseäsi peiliin.',
      'Pikimustat tähdet tuijottaa takaisin.',
      'Toivot, että joku huomaisi sinutkin.',
      'Pöydässäsi ei oo yhtään ystävää.',
      'Kaikki tietää sun tuhkimotarinas nimeltään.',
      'Ennen taivas oli avoinna sinullekin,',
      'kunnes paha sormi tavoitti karkurin.',
      '__KERTO:__',
      '>>Ja elämä on helppoo silloin, kun on joku,',
      '>>josta pitää kii.',
      '>>Ei tarvitse mennä nukkumaan itkeäkseen',
      '>>itsensä unelmiin.',
      '>>Ja elämä on päivästä päivään vieraiden',
      '>>pilkkaa sinullekin.',
      '>>Päivästä päivään levoton tuhkimo tekee',
      '>>itsestään marttyyrin.',
      'Illan värivalot on kaikille juhlaa (jaa jaa jaa).',
      'Kynttilöitä syttyy ja pidetään hauskaa.',
      'Vasen jalkasi hakkaa rytmissä musiikin.',
      'Valot eivät vain loista kasvoille sankarin.',
      'Ilta pimenee ja ihmiset lähtee pois.',
      'Sä et lähtis nyt kun aikaakin vielä ois.',
      'Ulkona sua oottaa lohduton huominen.',
      'Rakkauden hinta on nimensä veroinen.',
      'KERTO x2'
    ],
    title: 'Levoton Tuhkimo'
  },
  {
    metadata: ['(Esit. Hector)'],
    number: '318',
    lyrics: [
      'Tyttö näki sillalta kuinka mustaa',
      'vesi oli alhaalla jossakin.',
      'Äiti kysyi "Miksi tuo runoja rustaa?',
      'Lapsi on kai päästänsä sekaisin"',
      'Tyttö itki hetken ja katsoi taakseen.',
      'Villinä soi huokaus kaupungin.',
      'Kuinka tämän tuntisi omaksi maakseen?',
      'Vesi oli mustaa kuin aikakin.',
      '>>Ja vesi oli mustaa.',
      '>>Äiti kulki mustissaan.',
      '>>Ja pappikin sai kahvia juodakseen.',
      '>>Ja isä oli kännissä, kuinkas muutenkaan?',
      '>>Joku kantoi kukkia eteiseen.',
      'Toiset lähtee kuuhun ja toiset Ruotsiin.',
      'Toisilla vaan pienempi palkka on.',
      'Isä lähti viis vuotta sitten Ruotsiin.',
      'Äiti oli silloin jo onneton.',
      'Minä panin ikkunaan pahvisuojan,',
      'että edes jossakin lämmin ois.',
      'Radiossa laulettiin "Kiitos Luojan!"',
      'Sunnuntaina äitikin nukkui pois.',
      '>>:,: Ja isä meni Ruotsiin,',
      '>>äiti lensi taivaaseen.',
      '>>Ja pappi sai taas kahvia juodakseen.',
      '>>Ja veli oli kännissä',
      '>>näin sen itkeneen',
      '>>Lumi teki enkelin eteiseen. :,:'
    ],
    title: 'Lumi teki enkelin eteiseen'
  },
  {
    metadata: ['(Esit. Miljoonasade)'],
    number: '319',
    lyrics: [
      'Marraskuu,',
      'eilen torin laitaan kasvoi mandoliinipuu',
      'Marraskuu,',
      'nyt on sen kuoreen kirjoitettu "Beibi I love you"',
      'Mä kapakassa juopottelin sylissäni dinosauriin luu',
      'Marraskuu,',
      'Tänään huoneeni mä sotkin kun sua taas niin ikävöin',
      'Nyt kaipuun kaatopaikkaa täällä Kuivakurkku rotta isännöi',
      '__KERTO:__',
      '>>Ohi syyskuun, läpi repaleisen lokakuun',
      '>>kaipuun. Kaljakori kilisee,',
      '>>Yli taivaan',
      '>>päivät niin kuin varisparvi raahautuu',
      '>>Mua vaivaa ikävistä ikävin,',
      '>>milloin, baby, palaat takaisin.',
      '>>Etkö tiedä,',
      '>>voi yksinisen miehen viedä marraskuu',
      'Marraskuu,',
      'taas TV syliin paiskoo palavia halkojaan',
      'Marraskuu',
      'käy ihmiskunta sormi suussa kansakouluaan',
      'Vaan eihän täällä liekit uhkaa - ',
      'täällä pikkuhiljaa pohjaan poltetaan',
      'Marraskuu,',
      'Kaiken tämän keskellä sua hiljaa ikävöin',
      'Mä topin kanssa kellarilla ristinollaa',
      'pelaan päivin öin',
      'KERTO'
    ],
    title: 'Marraskuu'
  },
  {
    metadata: ['(Esit. Eppu Normaali)'],
    number: '320',
    lyrics: [
      'Syyttömänä syntymään sattui hän',
      'tähän maahan pohjoiseen ja kylmään,',
      'jossa jo esi-isät - juovuksissa tottakai - ',
      'hakkasivat vaimot, lapset jos ne kiinni sai.',
      'Perinteisen miehen kohtalon',
      "halus' välttää poika tuo.",
      'En koskaan osta kirvestä,',
      'enkä koskaan viinaa juo',
      'muuten juon talon.',
      'Lumihanki kutsuu perhettä talvisin,',
      'vaan en tahdo tehdä koskaan lailla isin',
      'Mut kun työnvälityksestä työtä ei saa,',
      'hälle kohtalon koura juottaa väkijuomaa',
      'Niin Turmiolan Tommi taas herää henkiin',
      'ja herrojen elkeet tarttuvat renkiin:',
      'Kohti laukkaa viinakauppaa.',
      'Se miehen epätoivoon ajaa,',
      'kun halla viljaa korjaa.',
      'Keskeltä kylmän mullan hiljaa',
      'kylmä silmä tuijottaa',
      'kun kirves kohoaa.',
      'Keskeltä kumpujen, mullasta maan',
      'isät ylpeinä katsovat poikiaan.',
      'Työttömyys, viina, kirves ja perhe',
      'lumihanki, poliisi ja viimeinen erhe.',
      'Tämä tuhansien murheellisten laulujen maa',
      'jonka tuhansiin järviin juosta saa.',
      'Katajainen kansa, jonka itsesäälin määrää',
      'ei mittaa järki, eikä Kärki määrää,',
      'jonka lauluissa hukkuvat elämän valttikortit',
      'ja kiinni pysyvät taivahan portit,',
      'Einari Epätoivosta ne kertovat.'
    ],
    title: 'Murheellisten laulujen maa'
  },
  {
    metadata: ['(Esit. Eppu Normaali)'],
    number: '321',
    lyrics: [
      'Voi mua väsynyttä rellestäjää,',
      'osaanko kotiini tulla?',
      'Maailmalle mitä musta pellestä jää,',
      'vain karmeita tarinoita repussa mulla.',
      'Nyt istun alas, panen reppuni laulamaan,',
      'tarinaa kertomaan, tarinaa menneistä, aa, markoista penneistä.',
      '__KERTO:__',
      '>>Nyt reppu jupiset riimisi rupiset',
      '>>tai katkon sinulta nyörit.',
      '>>Nyt korvaani karmeat juttusi supiset,',
      '>>kotona turhaan jaloissa pyörit.',
      '>>Et jättää saa pois sanaakaan.',
      '>>Et sitä kaunistakaan.',
      'Ei ruoho kasva missä kulkenut oon,',
      'paljon on surua tuonut.',
      'Tyttöä monta syliin sulkenut oon,',
      'karhuviinaa ja kahvia juonut.',
      'Palaa mieleni menneisiin muistan kun uhosin:',
      '"Sinne ja takaisin, vasta ukkona makaisin!"',
      '__KERTO__',
      'Ei mua revi enää reppuni tuo',
      'taittamaan taivalta teille.',
      'Viimein palaan kotikorjaamon luo',
      'viimein laulan lauluni teille.',
      'Ratapihalla nielen uudestaan ja uudestaan.',
      'Takerrun vaihteeseen samaan ja ruosteiseen.',
      'Sitä kääntelen edestakas.',
      'KERTO x2'
    ],
    title: 'Nyt reppuni jupiset riimisi rupiset'
  },
  {
    metadata: ['(Esit. Raptori)'],
    number: '322',
    lyrics: [
      'Oi, beibe, mä rakastan sua oikein kovasti,',
      'miksi ihmeessä isäsi on rovasti?',
      'Muuten toisiamme suutelisimme,',
      'auringonlaskuun ajaisimme.',
      'Olet kaunis ja minä voimakas,',
      'ah, sulla on ihana varvas!',
      'Haluaisin kovasti riisua paitasi,',
      'nähdä ihanat rusketusraitasi.',
      'Haluaisin suudella sinua kovasti,',
      'miksi ihmeessä isäsi on rovasti?',
      'On kaunis kesäyö,',
      'ei haittaa meitä koulutyö.',
      'Tulen aina muistamaan sut,',
      'toivottavasti säkin myös mut.',
      'Beibi, teen mitä tahansa vuoksesi,',
      'pakastan mummoni, lennän luoksesi.',
      '__KERTO:__',
      '>>Tahtoisin olla sun kanssasi sillai,',
      '>>tiedäthän kyllä, itsekkin millai.',
      '>>Tahtoisin sua niin koskettaa vaan,',
      '>>entä jos faijasi tietää saa?',
      'Kun sut nään housuissa sykkii,',
      'oi, beibe, tsiigaa tätä tykkii.',
      'Ah, nyt erittyy hormoonii,',
      'miksi täytyy faijasi olla mormoni?',
      'Ah, kuinka onkaan ihanat muotosi,',
      'haluaisin kokonaan omakseni ruotosi.',
      'Huvittaisi kyllä kanssasi yhtyä,',
      'mutta en viitsisi tappoon ryhtyä.',
      '__KERTO__',
      'Tahtoisin sua rakastella,',
      'muuten mua kutsuu kaasuhella,',
      'hirttoköysi tai partaterä.',
      'Tule tänne beibe, otetaan erä.',
      'Ah, kuinka onkaan ihana hajusi,',
      'ihmettelen kuinka faijasi tajusi,',
      'mitä silloin kylpyhuoneessa teimme,',
      'kun toisiltamme neitsyyden veimme.',
      '__KERTO__',
      'Oi, beibe, nyt taisin keksiä,',
      'voisimme kokeilla ryhmäseksiä.',
      'Otetaan faijasi mukaan kimppaan,',
      'olenkin jo kyllästynyt pelkkään pimppaan.',
      'Pienenä eno minut aina raiskasi',
      'jos en antanut seinään paiskasi.',
      'Ehkä juuri siksi olen tämänlainen,',
      'beibe, sä oot mun elämän nainen.',
      'KERTO x2'
    ],
    title: 'Oi beibi'
  },
  {
    metadata: ['(Esit. Black Sabbath)'],
    number: '323',
    lyrics: [
      "Finished with my woman 'cause she couldn't help me with my mind.",
      "People think I'm insane because I am frowning all the time.",
      'All day long I think of things but nothing seems to satisfy.',
      "Think I'll lose my mind if I don't find something to pacify.",
      'Can you help me? Are you for my brain? Oh yeah!',
      "I need someone to show me the things in life that I can't find.",
      "I can't see the things that make true happiness I must be blind.",
      'Make a joke and I will sigh and you will laugh and I will cry.',
      'Happiness I cannot feel so love to me is so unreal.',
      'And so as you hear these words telling you now of my state.',
      "I tell you to enjoy life I wish I could but it's too late."
    ],
    title: 'Paranoid'
  },
  {
    metadata: ['(Esit. Eppu Normaali)'],
    number: '324',
    lyrics: [
      'Omistettu TYY:lle',
      'Suomalaiset ovat hyvin harrasta kansaa,',
      'ne kirkonmenot aina aikaan saa.',
      'Oli seurat, sirkus taikka huumoria hurttia,',
      'on naamat kuin ois syöty yoghurttia.',
      '__KERTO:__',
      '>>:,: Päpädädäpäädäppäpää',
      '>>nyt pidetään ikävää',
      '>>TytydydyTYYdyttyTYY.',
      '>>Niin sielu TYYdytTYY. :,:',
      'Kun punk-kohkari punk-kohkasi,',
      'hän mainiot seurat siitä lohkasi.',
      'Ja kulttuurifriikki huusi: "ylös sanskriitti!"',
      'On pyhä riitti, tietty körttiläisbeatti',
      '__KERTO__',
      'On paljon hauskempi olla pa-pa-pakanana,',
      'ei tartte koskaan olla va-va-vakavana.',
      'Mut on hauskempi olla pa-pa-pakanana,',
      'ei tartte koskaan olla va-va-vakavana.',
      'KERTO'
    ],
    title: 'Pidetään ikävää'
  },
  {
    metadata: ['(Säv. Anssi Kelan Milla)'],
    number: '325',
    lyrics: [
      'Mul on sängyssäni mies,',
      'sen olkaan tatuoitu on Jack Nicholson.',
      'Se makaa alasti ja nukkuu',
      'ja mä mietin kuka helvetti tuo on.',
      'Ei ainakaan se sadun fuksi,',
      'se johon tulisesti rakastun.',
      'Joten nousen vähin äänin,',
      'porraskäytävässä pukeudun.',
      'Otan Trebarista pöydän,',
      'selkä ulko-ovelle päin.',
      'Enkä todellakaan tunne',
      'minkäänlaista ylpeyttä itsestäin.',
      'Proffa soittaa kertoakseen,',
      'miten kiva ois jos joskus kävisin.',
      'Mut se veisais vain sen saman virren,',
      'se on kuultu monta kertaa ennenkin.',
      'Se menee näin:',
      ':,: Piltti hei sun täytyy muuttuu.',
      'Sun elämältäs suunta puuttuu,',
      'sä aina tartut tuoppiin täydempään.',
      'Se ei johda mihinkään.',
      'Sä et oo mikään abi enää,',
      'sun täytyis tehdä jotain järkveää.',
      'Ja lapselliset leikkis lopettaa. :,:'
    ],
    title: 'Piltti'
  },
  {
    metadata: ['(Esit. Leevi and the Leavings)'],
    number: '326',
    lyrics: [
      'Minä halusin tavata Annelin,',
      'sillä tiesin sen minua vielä etsivän.',
      'Eihän elämä paljoa antanut.',
      'Sateenkaari päättyi nakkikioskille,',
      'ravintolapöytään oksentaville naurettiin.',
      '__KERTO:__',
      '>>O-o-oo-o-o-o.',
      '>>Dam di-di-dam di-di-dam di-di-dam-dam-dam-dam-dam.',
      '>>Di-di-dam di-di-dam di-di-dam-dam-dam-dam-dam.',
      '>>Minä lähden Pohjois-Karjalaan.',
      '>>Vaihdan farkut verkkarihousuun.',
      '>>Kotiseudulle Pohjois-Karjalaan.',
      '>>Juon kaljaa auringon nousuun.',
      'Sivukujilla elämän kulkien',
      'toivon onneni jostain vielä löytyvän.',
      'Enhän minäkään liikoja vaatisi,',
      'jotakin jolla tarjoaisin illan,',
      'elatusmaksut lottokupongilla kuittaisin.',
      '__KERTO__',
      'Vaikka elämän sävelen hukkasin',
      'oman onneni sirpaleita säilytän.',
      'Sillä sinua kaltoin jos kohtelin,',
      'olisin voinut vaikeampi olla.',
      'Yritin silti aina sovinnolla kuitenkin.',
      'KERTO'
    ],
    title: 'Pohjois-Karjala'
  },
  {
    metadata: ['(Esit. Tapio Rautavaara)'],
    number: '328',
    lyrics: [
      'Aurinko kun päätti retken,',
      'siskoistaan jäi jälkeen hetken',
      'päivänsäde viimeinen.',
      'Hämärä jo metsään hiipi,',
      'päivänsäde kultasiipi',
      'juuri aikoi lentää eestä sen,',
      'kun menninkäisen pienen näki vastaan tulevan;',
      'se juuri oli noussut luolastaan',
      'Kas menninkäinen ennen päivän laskua ei voi',
      'milloinkaan olla päällä maan.',
      'Katselivat toisiansa -',
      'menninkäinen rinnassansa',
      'tunsi kummaa leiskuntaa.',
      "Sanoi: 'Poltat silmiäni,",
      'mut en ole eläissäni',
      'nähnyt mitään yhtä ihanaa!',
      'Ei haittaa, vaikka loisteesi mun sokeaksi saa -',
      'on pimeässä hyvä asustaa.',
      'Käy kanssani, niin kotiluolaan näytän sulle tien -',
      "ja sinut armaakseni vien!'",
      "Säde vastas: 'Peikko kulta,",
      'pimeys vie hengen multa,',
      'enkä toivo kuolemaa.',
      'Pois mun täytyy heti mennä,',
      'ellen kohta valoon lennä,',
      "niin en hetkeäkään elää saa!'",
      'Niin lähti kaunis päivänsäde, mutta vieläkin,',
      'kun menninkäinen öisin tallustaa,',
      'hän miettii, miksi toinen täällä valon lapsi on,',
      'ja toinen yötä rakastaa.'
    ],
    title: 'Päivänsäde ja menninkäinen'
  },
  {
    metadata: ['(Esit. Hassisen kone)'],
    number: '329',
    lyrics: [
      'Sinä kuuntelet tv-tuolissas',
      'Minun paskaista ääntäni',
      'ja olet aivan liian laiska,',
      'Jotta kanavan toiselle kääntäisit.',
      'Vaimollesi huudat: "Tuo sitä kaljaa,',
      'Eihän tätä kestä selvinpäin,',
      'kun huligaanit huutaa ja rapparit räyhää',
      'ja elämä on muutenkin niin läyhää"',
      '__KERTO:__',
      '>>Jos rappio on sitä, että rakastaa,',
      '>>ottaa hetkistä kii, eikä tahdo tappaa,',
      '>>niin on mukavaa olla rappiolla,',
      '>>olla vaan, täysi nolla!',
      '>>Rappiolla on hyvä olla,',
      '>>ei huolet paina, ei rasitu polla.',
      '>>Antaudu sinäkin suosiolla,',
      '>>taikka anna meidän olla!',
      'ÄO:ni on kuulemma',
      'Normaalia parempi.',
      'Miksi kouluja pelkään?',
      'Olenko muita arempi?',
      'Vitut minä mikään viisas ole,',
      'tahdon elää vain kuin ihminen.',
      'Päästä irti isoveli,',
      'kuule kutsua jumalten!',
      '__KERTO x2__',
      'Anna meidän olla',
      'anna meidän olla',
      'anna meidän olla',
      'anna meidän OLLA!'
    ],
    title: 'Rappiolla'
  },
  {
    metadata: [],
    number: '330',
    lyrics: [
      'Tässä tulee hirmuinen Rölli!',
      'On mulla metsässä tölli.',
      'Ja siellä mä asustelen,',
      'ja sieltä mä hiippailen.',
      'Tilipitäppi-tippi-täppi,',
      'tilipitäppi-tippi-täppi,',
      'tilipitäppi-tippi-täppi, joo.',
      'Mä varjoissa hiiviskelen,',
      'ja lapsia säikyttelen.',
      'Ja se on vasta mukavaa,',
      'kun kaikkia pelottaa.',
      '__KERTO__',
      '>>Tilipitäppi-tippi-täppi,',
      '>>tilipitäppi-tippi-täppi,',
      '>>tippi-täppi-tippi-täppi, joo.',
      'En ole minä määkivä lammas,',
      'ja suussani yksi iso hammas.',
      'Ja sillä mä irvistelen,',
      'ja kaikkia säikyttelen.',
      'KERTO x3'
    ],
    title: 'Rölli'
  },
  {
    metadata: ['(Esit. Ultra Bra)'],
    number: '331',
    lyrics: [
      'Aamuyöllä kumpikaan meistä ei jaksa enää juhlia.',
      'Aamuyöllä kumpikaan meistä ei jaksa enää juhlia.',
      'Väsyneet katseet, käheät äänet.',
      'Taustalla aurinko joka ei lämmitä,',
      'taustalla aurinko joka ei lämmitä.',
      '__KERTO:__',
      '>>Sinä lähdit pois',
      '>>minä katselin parvekkeelta',
      '>>loittonevaa selkääsi',
      '>>kiersit vesilammikot',
      '>>ja arvaan ettet murehdi tätä eroa',
      'Pensaiden kaikki laulavat linnut aamulla ennen seitsemää.',
      'Pensaiden kaikki laulavat linnut aamulla ennen seitsemää.',
      'Väsyneet katseet, käheät äänet. Taustalla aurinko joka ei lämmitä,',
      'taustalla aurinko joka ei lämmitä.',
      'Sinä lähdit pois...',
      'Yön viimeinen drinkki',
      'aamiaiseksi vaihtuu,',
      'lisäsin siihen kahvia.',
      '__KERTO x2__',
      'Eroa,',
      'eroa...'
    ],
    title: 'Sinä lähdit pois'
  },
  {
    metadata: ['(Esit. Juice Leskinen)'],
    number: '332',
    lyrics: [
      'Katu täytyy askelista, elämä on kuolemista.',
      'Pane käsi käteen, ollaan hiljaa.',
      'Pyydä minut aamuteelle, anna vettä kuihtuneelle.',
      'Nyt on elokuu ja minä olen viljaa',
      '__KERTO__',
      'En ilosta itke en surusta itke,',
      'jos itken niin itken muuten vaan,',
      'ja muualla oon, ennen kuin huomaatkaan.',
      'Häntä rakastin paljon, sua rakastan joskus enemmän,',
      'ole mulle vähän aikaa hän.',
      'Ota minut sinun uniin, vaikka nousen toisiin juniin',
      'Nyt on lokakuu ja minusta näkee sen',
      'Kun tulen kiinni sinuun, jumalatkin uskoo minuun,',
      'vaikka itse aina usko en.',
      'KERTO'
    ],
    title: 'Syksyn sävel'
  },
  {
    metadata: ['(Esit. Freud, Marx & Engels Jung)'],
    number: '333',
    lyrics: [
      'Särkynytsydämiset aina juovan tuntuvat,',
      'hukuttaakseen murheet, jotka kelluu kuin untuvat.',
      'Ja ehkä elämä on vain huonoa pilaa, mutta mehän hekotetaan.',
      'Niin, että kukahan seuraavat tilaa, sillä kai me ne otetaan.',
      '__KERTO:__',
      '>>Viekää minut baariin veljet, tuoliin nostakaa.',
      '>>Hitto joo, mä en pelle oo, jos mies en olla saa.',
      'Älä kysy, mikä mul on, minä tiesin mitä mä tein',
      'joka kerta, kun pullon Vanhan nurkkapöytään vein.',
      'Ja elämä tuntui ihanalta, mutta hyvältä näyttänyt en,',
      'sillä piinatun lihan alta, mä olin syvältä käyttänyt sen.',
      '__KERTO__',
      'Joten jostain taksi hoida, ei jäädä aikaa haaskaamaan.',
      'Nyt ei oo aikaa valikoida - mihin tahansa kapakkaan.',
      'Eli kuski, ala jo ajaa kohti lähintä ravintolaa.',
      'Minä etsin vapahtajaa, etsin lisää humalaa.',
      '__KERTO__',
      'Viekää minut baariin veljet, tuoppi ostakaa.',
      'Hitto joo, mä en pelle oo, jos mies en olla saa.',
      'KERTO'
    ],
    title: 'Särkynytsydämiset'
  },
  {
    metadata: ['(Esit. Eppu Normaali)'],
    number: '334',
    lyrics: [
      'Tiedän että on yo kun saavut kotiisi pimeään.',
      'Yksinäisyys lyö sanomatta nimeään.',
      'Kun luet kirjeeni kaiun kuulet,',
      'tahrat näet joita sateeksi luulet.',
      'Kuljen ulkona valossa, kasvoillani voin tuntea tuulen.',
      'Ei sitä tuntunut talossa, johon palaa en koskaan, luulen.',
      'En ole menossa minnekkään.',
      'Ei missään tule minun luokseni, minun vuokseni.',
      '__KERTO:__',
      '>>Vain tahroja paperilla älä siis suutu,',
      '>>ei niistä asiat miksikäänmuutu.',
      '>>Ei se, että meillä oli retkemme,',
      '>>eikä se, etta meillä oli hetkemme.',
      '>>Voi tuuli kylmästi kutittaa selkään,',
      '>>se eteenpäin työntää, älä siis pelkää.',
      '>>Älä huoli siitä, sillä meillä oli hetkemme, aha.',
      '>>Muista, että meillä oli hetkemme.',
      'Kutsun saaneena tulin, lähdin kuin varas, säikähdin viimein.',
      'Kun itseni tarttuneen, tajusin varttuneen.',
      'Imin sut kuiviin, tarttui mun huiviin,',
      'tuoksusi, teit saman minulle, juoksusi, annan anteeksi sinulle.',
      'KERTO x2'
    ],
    title: 'Tahroja paperilla'
  },
  {
    metadata: ['(Esit. Tapio Rautavaara)'],
    number: '335',
    lyrics: [
      'Moneen tuoppiin jo kourani tarttua sai,',
      'tämän kirjavan kulkuni tiellä.',
      'Moni tuoppini hauskasti tyhjeni kai,',
      'kait se katkera ollut on niellä.',
      'Joka tuopista jälkiä pöytään jää',
      'ja ne montakin muistoa säilyttää.',
      'Itse tuoppini jäljet mä tunnen.',
      'Moni tyhjeni seurassa ystävien,',
      'kuvat hilpeät vaahdossa kulki.',
      'Monen muisto on kylmä ja murheellinen,',
      'suru suuni kun laululta sulki.',
      'Jälki harmaa jos tuopista jäädä vois,',
      'paras maksaa on lasku ja mennä pois.',
      'Itse tuoppini jäljet mä tunnen.',
      'Toki tuoppini kuohussa välkkynyt on',
      'usein muisto niin lämmin ja hellä.',
      'Tuopin sellaisen muisto on kuolematon,',
      'hyvä silloin on mulla, jos kellä.',
      'Jälki tuoppini rattoisa silloin lie.',
      'Se ei syytä, vaan herkäksi mielen vie.',
      'Itse tuoppini jäljet mä tunnen.',
      'Elon krouveissa vaihteluun tottua saa,',
      'tuoppi näyttää voi houkuttavalta.',
      'Vaikka raikasta on se ja vaahtoavaa,',
      'sakan löytää voi kielensä alta.',
      'Jälki pöydässä ilmoittaa mulle sen,',
      'että pettymysisku on melkoinen.',
      'Itse tuoppini jäljet mä tunnen.',
      'Mutta tuoppini tyhjennän, vaikka en voi',
      'kuohun laatua arvata aina.',
      'Ilo ehkäpä tuopissa vaahtona soi,',
      'tuoppi silloin ei kourassa paina.',
      'Tumman jäljen jos tuoppini pöytään tuo,',
      'tuoppi uusi jo ehkäpä kirkkaan luo.',
      'Itse tuoppini jäljet mä tunnen.'
    ],
    title: 'Tuopin jäljet'
  },
  {
    metadata: ['(Esit. Leevi and the Leavings)'],
    number: '336',
    lyrics: [
      'Maanantai-aamuna krapula ja vapina, tuijotan vessan seinää.',
      'Olen työpaikan sonni, porsas sekä apina, suu täynnä kuivaa heinää.',
      'Silti toimiston tyttöjen salaisissa unissa kaikkien kanssa vehtaan.',
      'Tunnen hankaussähköä, kipinöitä munissa, minä se vasta kehtaan...',
      '__olla mies...__',
      '>>Nainen, tahdotko että sut pois näistä kuvioista veisin?',
      '>>Jos haluat, voit vaikka istua kasvojen päälle hajareisin.',
      '>>Ja syviä naarmuja raapia selkääni punaisilla kynsillä,',
      '>>puhua kielillä, kiusata hengiltä tekemällä kaiken ...kaiken.',
      '_Ja hela hoito uudestaan ja uudestaan_'
    ],
    title: 'Unelmia ja toimistohommia'
  },
  {
    metadata: ['(Esit. Juice Leskinen)'],
    number: '337',
    lyrics: [
      'Silmissäni orpo katse lapsen eksyneen.',
      'Rakkautesi jäljet vihloo rintaa.',
      'Käy pimeys päälle lailla pommikonelaivueen.',
      'Ja minä kun en koskaan kysy hintaa!',
      'Viidestoista yö taas saapuu painajaisineen,',
      'ja kaikki, paitsi elämä, on turhaa.',
      'Aina oikein uskoa ei jaksa ihmiseen,',
      'kun ihmiskunta tekee itsemurhaa.',
      '__KERTO:__',
      '>>Sun kanssas katson maailmaa ja samaa unta nään',
      '>>Kuin hullu huudan rakkauteni perään!',
      '>>Sun kainaloosi käperryn, ja jos sallit, yöksi jään,',
      '>>ja kun aamu on, en tiedä missä herään.',
      'Käy tuuli läpi pääni etelästä pohjoiseen,',
      'oon yksin ajatusten raunioilla.',
      'Ikuisesti kolisevat sanat tyhjyyteen,',
      'On jälleen tuskan viitta hartioilla.',
      '__KERTO__',
      'Viidestoista yö, se saapuu aina uudelleen;',
      'taas huomaan kahden viikon unohtuneen.',
      'Jäi iholleni hiekkaa rannan autioituneen,',
      'voi olla että vihdoin pääsen uneen.',
      'KERTO x2'
    ],
    title: 'Viidestoista yö'
  },
  {
    metadata: ['(Esit. Queen)'],
    number: '338',
    lyrics: [
      'Omistettu kaikille maailman herkkusienille',
      "I've paid my dues",
      'time after time.',
      "I've done my sentence",
      'but committed no crime.',
      'And bad mistakes',
      "I've made a few",
      "I've had my share of sand kicked in my face",
      "but I've come through.",
      'And I need to go on and on and on and on.',
      '__KERTO:__',
      '>>We are the champions, my friend.',
      ">>And we'll keep on fighting till the end.",
      '>>We are the champions,',
      '>>we are the champions.',
      '>>No time for losers',
      ">>'Cause we are the champions",
      '>>of the world.',
      "I've taken my bows",
      'And my curtain calls.',
      "You've brought me fame and fortune",
      'and everything that goes with it',
      'I thank you all',
      "But it's been no bed of roses, no pleasure cruise.",
      'I consider it a challenge before the whole human race',
      "and I'd never lose.",
      'And I need to go on and on and on and on.',
      'KERTO x2'
    ],
    title: 'We are the Champions'
  },
  {
    metadata: ['(Säv. Moskau)'],
    number: '339',
    lyrics: [
      'Volga - rantaa sen ratsastaa Ivan ja Katjusha,pitkä on tie.',
      'Volga - pelasta rakkaus, näytä sen oikeus, turvaan se vie.',
      'Nyt Ivan piiskaa ratsuaan juoksuun, (Hei!)',
      'jo tsaarin miehet kannoilla on (Ha!)',
      'ja pikku-Katjan hiusten tuoksuun (Hei, hei!)',
      'sekoittuu pelko niin mittaamaton (Hei, hei, hei, hei!)',
      'Volga, Volga - rakkaus tai kuolema,',
      'pelkoa ja vainoa, ho-ho-ho-ho-ho-hei!',
      'Volga, Volga - villinä ja vapaana,',
      'sua ei voi vangita, ha-ha-ha-ha-ha-hei!',
      'Volga, Volga - rakkaus tai kuolema,',
      'pelkoa ja vainoa, ho-ho-ho-ho-ho-hei!',
      'Volga, Volga - näytä meille Kaspia,',
      'siellä oomme turvassa, ha-ha-ha-ha-ha-ha!',
      'Volga - yötä vain odottaa Ivan ja Katjusha uuvuksissaan.',
      'Volga - ratsu jo huohottaa, täytyykö luovuttaa? Ei milloinkaan!',
      'He eivät koskaan näkisi merta, (Hei!)',
      'siis Ivan kääntyy hyökkääjää päin (Ha!)',
      'ja Volgan liejuun sekoittuu verta, (Hei, hei!)',
      'sen virta kuljettaa Kaspiaan näin (Hei, hei, hei, hei!)',
      'Volga, Volga - rakkaus tai kuolema,',
      'pelkoa ja vainoa, ho-ho-ho-ho-ho-hei!',
      'Volga, Volga - villinä ja vapaana,',
      'sua ei voi vangita, ha-ha-ha-ha-ha-hei!',
      'Volgaaa - laa-la-la-la-la-la-la,',
      'laa-la-la-la-la-la-la, ho-ho-ho-ho-ho-hei!',
      'Volgaaa - laa-la-la-la-la-la-la,',
      'laa-la-la-la-la-la-la, ha-ha-ha-ha-ha-ha!',
      'Volga! Volga!',
      'Volga, Volga - rakkaus tai kuolema,',
      'pelkoa ja vainoa, ho-ho-ho-ho-ho-hei!',
      'Volga, Volga - villinä ja vapaana,',
      'sua ei voi vangita, ha-ha-ha-ha-ha-hei!',
      'Volga! Volga!',
      'Jos emme koskaan näkisi merta, (Hei!)',
      'niin jäätkö silloin vierellein mun? (Ha!)',
      'Jos emme koskaan näkisi merta, (Hei, hei!)',
      'niin loppuun asti mä aina oon sun (Hei, hei, hei, hei!)',
      ':,: Volga, Volga - rakkaus tai kuolema,',
      'pelkoa ja vainoa, ho-ho-ho-ho-ho-hei!',
      'Volga, Volga - villinä ja vapaana,',
      'sua ei voi vangita, ha-ha-ha-ha-ha-hei! :,:'
    ],
    title: 'Volga'
  },
  {
    metadata: [],
    number: '401',
    lyrics: [
      '__SUOMI:__',
      'Enkeli taivaan lausui näin:',
      "Miks' hämmästyitte säikähtäin?",
      'Mä suuren ilon ilmoitan',
      'maan kansoille nyt tulevan!',
      'Herramme Kristus teille nyt',
      'on tänään tänne syntynyt,',
      'ja tää on teille merkiksi:',
      'seimessä lapsi makaapi',
      'Nyt Jumalalle kunnia,',
      'kun antoi ainoon Poikansa.',
      "Siit' enkelitkin riemuiten",
      'veisaavat hälle kiitoksen',
      '__SAKSA:__',
      "Vom Himmel hoch da komm' ich her,",
      'ich bring euch gute, neue Mär,',
      "der guten Mär bring' ich so viel,",
      "davon ich sing'n und sagen will."
    ],
    title: 'Enkeli taivaan - Vom Himmel Hoch'
  },
  {
    metadata: ['Keksinet varmaan pitää tehdä Nyt:n kohdalla...'],
    number: '402',
    lyrics: [
      'No, onkos tullut kesä',
      'nyt - talven keskelle?',
      'Ja laitetaankos pesä',
      'nyt - pikkulinnuille?',
      'On kuusi kynttilöitä',
      'nyt - käynyt kukkimaan',
      'pimeitä talven öitä',
      'nyt - ehkä valaistaan',
      'Ja vanhakin nyt - nuortuu',
      'kuin lapsi leikkimään, nyt -',
      'ja koukkuselkä nyt - suortuu,',
      "niin kaikk' nyt - on mielissään, nyt - nyt - ..."
    ],
    title: 'No onkos tullut - nyt'
  },
  {
    metadata: [],
    number: '403',
    lyrics: [
      'Muistat Tuhkimon, Lumikin, Ruususen varmaan',
      'ja punahilkan ja sudenkin harmaan,',
      'mutta poro tää sulta usein unhoon jää',
      'Petteri Punakuono oli poro nimeltään',
      'Ollut ei loiste huono Petterillä nenänpään',
      'Haukkuivat toiset illoin majakaksi pilkaten',
      'Tuosta vain saikin silloin joulupukki aattehen',
      'Aattoilta pitkä on, taival valoton',
      'Petteri vois nenässään valon tuoda pimeään',
      'Petteri siitä asti pulkkaa pukin kiskoen',
      'johtaa sen riemuisasti luokse lasten kilttien'
    ],
    title: 'Petteri Punakuono'
  },
  {
    metadata: [],
    number: '404',
    lyrics: [
      ':,: Joulu on taas, joulu on taas,',
      'kattilat täynnä puuroo :,:',
      ':,: Nyt sitä saa, nyt sitä saa',
      'vatsansa täyteen puuroo :,:',
      ':,: Joulu on taas, joulu on taas,',
      'voi, kuinka meillä on hauskaa! :,:',
      ':,: Lapsilla on, lapsilla on',
      'aamusta iltaan hauskaa :,:',
      ':,: Joulu on taas, joulu on taas,',
      'kannut on täynnä sahtii :,:',
      ':,: Nyt sitä saa, nyt sitä saa,',
      'vatsansa täyteen sahtii :,:',
      ':,: Joulu on taas, joulu on taas,',
      'voi kuinka meillä on hauskaa :,:',
      ':,: Meilläkin on, meilläkin on,',
      'lasit täynnänsä punssii! :,:'
    ],
    title: 'Puurolaulu'
  },
  {
    metadata: ['(Esit. Juice Leskinen)'],
    number: '407',
    lyrics: [
      'Joulun viettomme alkaa jo toukokuussa,',
      'porsaalla silloin on herkut suussa.',
      'Se lihoo, se kasvaa, se vahvistuu,',
      'ja sitten kun koittaa marraskuu,',
      'niin lapset laulaa:',
      "'Joulun tähti on ehdoton'.",
      '>>Sika - se kuulan kalloon saa.',
      '>>Sika - sen setä teurastaa.',
      '>>Sika - ja setä verta juo.',
      '>>Sika - se joulumielen tuo.',
      'Minä läävässä lojuvaa karjua katsoin',
      'kärsivin ilmein, vellovin vatsoin.',
      'Jouluna sikaa mä muista en,',
      'vain kinkkua vahtaan himoiten.',
      'Ja lapset laulaa:',
      "'Joulun tähti on ehdoton'.",
      '>>Sika - kun sitä suolataan.',
      '>>Sika - yhdessä kuolataan.',
      '>>Sika - ja isi mielellään.',
      '>>Sika - on vesi kielellään.',
      'Jouluruuhkassa ihminen sokkona ryntää,',
      'kadulla adventtisohjoa kyntää.',
      'Joku ihmistä katsoo, kääntää pään,',
      'kun ihminen rypee läävässään.',
      'Ja lapset laulaa:',
      "'Joulun tähti on ehdoton'.",
      '>>Sika - se kohta paistetaan.',
      '>>Sika - sen perää maistetaan.',
      '>>Sika - on sähköuunissa.',
      '>>Sika - on mutsi duunissa.',
      'Kato, äiti on laittanut kystä kyllä,',
      'sinappihuntu on sialla yllä.',
      'Isoveli veistänsä terottaa',
      'ja luistansa porsaan erottaa.',
      'Ja lapset laulaa:',
      "'Joulun tähti on ehdoton'.",
      '>>Sika - lopulta tappoi sen.',
      '>>Sika - ympäristö vatsan happoisen.',
      '>>Sika - voi kuinka isi syö.',
      '>>Sika - isiltä loppuu vyö.',
      'Joulu täynnä on kinkkua, kylkeä, potkaa,',
      'läskiä, niskaa ja tietenkin votkaa.',
      'Kun loppuu aatto ja alkaa yö,',
      'niin ihminen on sitä, mitä hän syö:',
      '>>Sika - tralala laalalaa...',
      '_Toisteleppa tuota tarpeeksesi_'
    ],
    title: 'Sika'
  },
  {
    metadata: [],
    number: '409',
    lyrics: [
      'Soihdut sammuu, kaikki väki nukkuu, väki nukkuu.',
      'Öitten varjoon talon touhu hukkuu, touhu hukkuu.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Tonttujoukko silloin varpahillaan, varpahillaan,',
      'varovasti hiipi alta sillan, alta sillan.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Jouluruokaa tarjoo kunnon väki, kunnon väki.',
      'Raoistansa sen jo tonttu näki, tonttu näki.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Pöydän alle veitikkaiset rientää, veitikkaiset.',
      'Syövät paistia ja juovat lientä, juovat lientä.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Herkkua on siinä monenlaista, monenlaista.',
      'Kuiske kuuluu, miltä ruoka maistaa, ruoka maistaa.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Sitten leikitellään kuusen alla, kuusen alla,',
      'kunnes päivä koittaa taivahalla, taivahalla.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap',
      'Hiljaa hiipii joukko varpahillaan, varpahillaan,',
      'kotikoloihinsa alle sillan, alle sillan.',
      '>>Tip-tap, tip-tap, tipe-tipe tip-tap, tip, tip, tap'
    ],
    title: 'Tonttujen jouluyö'
  },
  {
    metadata: [],
    number: '408',
    lyrics: [
      'Lumi on jo peittänyt kukat laaksosessa,',
      'järven aalto jäätynyt talvipakkasessa.',
      'Varpunen pienoinen syönyt kesäeinehen.',
      'Järven aalto jäätynyt talvipakkasessa.',
      'Pienen pirtin portailla oli tyttökulta:',
      'Tule, varpu, riemulla, ota siemen multa!',
      'Joulu on, koditon varpuseni onneton,',
      'tule tänne riemulla, ota siemen multa!',
      'Tytön luo nyt riemuiten lensi varpukulta:',
      'Kiitollisna siemenen otan kyllä sulta!',
      'Palkita Jumala tahtoo kerran sinua.',
      'Kiitollisna siemenen otan kyllä sulta!',
      'En mä ole, lapseni, lintu tästä maasta,',
      'olen pieni veljesi, tulin taivahasta.',
      'Siemenen pienoisen, jonka annoit köyhällen,',
      'pieni sai sun veljesi enkeleitten maasta.'
    ],
    title: 'Varpunen jouluaamuna'
  },
  {
    metadata: ['(Säv. Lähteellä)'],
    number: '502',
    lyrics: [
      'Laulun jälkeen jokainen, Jjoka ei lasiansa ole tyhjentänyt, Kärsiköön peljättävän rangaistuksen',
      'En liten fågel satt en gång',
      'och sjöng i furuskog.',
      'Han hade sjungit dagen lång',
      'men dock ej sjungit nog.',
      'Vad sjöng den lilla fågeln då? Tjo!',
      'Helan går, sjung hoppfaderallanlallanlej,',
      'helan går, sjung hoppfaderallanlej!',
      'Och den som inte helan tar,',
      'han ej heller halvan får.',
      'Helan går! Sjung hoppfaderallanlallanlej!',
      '__SUOMI: POHJAAN SIIS__',
      'Pohjaan siis tää ensi malja juodahan,',
      "pohjaan siis, kyll' lisää tuodahan!",
      "Ensimmäist' ei purra saa,",
      'kukaan tääll ei surra saa!',
      'Pohjaan siis - tää malja juodahan!',
      'Pohjahan, hei, veikot, juodaan malja tää,',
      'pohjahan, hei, veikot, malja tää!',
      'Ken ei ryyppää pohjahan,',
      'ei saa ottaa tippaakaan!',
      'Pohjahan - hei veikot, malja tää!',
      'Pohjaan juo, nyt heippatirallallallallei,',
      'pohjaan juo, nyt heippatirallallei!',
      "Ken ensimmäist' ei kaadakaan,",
      'toista ei voi saadakaan!',
      'Pohjaan juo - nyt heippatirallallei!',
      '__ENGLANTI: HELL AND GORE__',
      'Hell and gore, shun gop Father Allan Allan ley,',
      'hell and gore, shun gop Father Allan ley.',
      'For handsome in the hell and tar',
      'then hell are in the half and four.',
      'Hell and gore - shun gop Father Allan ley!',
      '__TAKAPERO: NALEH RÅG__',
      'Naleh råg, gnujs ppoh nallaredaf nallar jel,',
      'naleh råg, gnujs ppoh nallaredaf jel.',
      'Hco ned mos etni naleh rat,',
      'nah je relleh navlah råf!',
      'Naleh råg, gnujs ppoh nallredaf jel!'
    ],
    title: 'Helan går'
  },
  {
    metadata: [],
    number: '504',
    lyrics: [
      'Jos (e)ukkosi kieltää sua juomasta,',
      'niin juo, niin juo!',
      'Jos kieltää sua viiniä maistamasta,',
      'niin tuo, niin tuo!',
      "Mutt' juomasta älä sinä milloinkaan lakkaa,",
      'vaan ota sinä itsellesi parempi akka (ukko)',
      'ja juo ja laula',
      'ja juo ja laula',
      'ja juo ja laula ja juo!',
      'Upseerit sodassa taistelee,',
      'ja juo, ja juo!',
      'Niin välillä pullosta maistelee,',
      'ja juo, ja juo!',
      'Kun taistelun melskeessä pyssyt ne paukkaa,',
      'niin upseerit välillä pullosta naukkaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa juo!',
      'Ja assarit ICT:n opettaa,',
      'ja juo, ja juo!',
      'Kun illalla luentonsa lopettaa,',
      'niin juo, niin juo!',
      'Kun päivällä TRAKkia ja OOPpia jauhaa,',
      'niin illalla raitilla räyhää ja pauhaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa ja juo!!!',
      'Kun olutta paljon he juoneet on, he on, he on,',
      'on rakossa paine jo mahdoton, niin on, niin on.',
      'Niin nestettä ulos tuli laskettavaksi,',
      'ja uutta voi naukata pullon tai kaksi',
      'ja juo ja laulaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa juo!',
      'Yhä seurassa veikot nuo maistelee, ja juo, ja juo.',
      'Ja viime pullosta taistelee, ja juo, ja juo.',
      'Kun baarissa viimein on tarjoilu loppu,',
      'on päässäkin sammunut viimeinen proppu',
      'ja juo ja laulaa,',
      'ja juo ja laulaa,',
      'ja juo ja laulaa juo!'
    ],
    title: 'Jos (e)ukkosi kieltää'
  },
  {
    metadata: [],
    number: '405',
    lyrics: [
      'En etsi valtaa loistoa, en kaipaa kultaakaan',
      'Mä pyydän taivaan valoa ja rauhaa päälle maan',
      'Se joulu suo, mi onnen tuo ja miehet nostaa Luojan luo',
      'ei valtaa eikä kultaakaan, vaan rauhaa päälle maan',
      'Suo mulle maja rauhaisa ja lasten juoulupuu',
      "Jumalan sanan valoa, joss' sieluin kirkastuu",
      'Tuo kotihin, jos pieneenkin, nyt joulujuhla suloisin,',
      'Jumalan sanan valoa ja mieltä jaloa',
      'Luo köyhän niinkuin rikkahan saa, joulu ihana',
      'Pimeytehen maailman suo taivaan valoa',
      'Sua halajan, sua odotan, sa Herra maan ja taivahan',
      'Nyt köyhän niinkuin rikkaan luo suloinen joulus tuo'
    ],
    title: 'Sibeliuksen joululaulu'
  },
  {
    metadata: [],
    number: '406',
    lyrics: [
      'Ja niin joulu joutui jo taas Pohjolaan;',
      'joului joutui jo rintoihinkin.',
      'Ja kuuset ne kirkkaasti luo loistoaan',
      'jo pirtteihin pienoisihin.',
      "Mutt' ylhäällä orressa vielä on vain",
      'se häkki, mi sulkee mun sirkuttajain.',
      'Ja vaiennut vaikerrus on vankilan;',
      "oi, murheita muistaa sen vois' laulajan?",
      "Miss' rannoilla loiskuvi aallot ne ain,",
      'missä hedelmät niin hehkuvat myös.',
      'Ja korkeina sypressit huojuu, mä sain',
      "siell' laulella lauhassa yöss'",
      'Kun liekit löy Etnasta taivaalle noin,',
      'kuin Eedenin autuuden siellä mä koin.',
      "Mut' vanki, jos oonkin nyt pakkasten maan,",
      'niin jouluna, jouluna laulaa ma saan!',
      'Sä tähdistä kirkkain nyt loisteesi luo',
      'sinne Suomeeni kaukaisehen Ja sitten, kun sammuu sun tuikkeesi tuo,',
      'sa siunaa se maa muistojen.',
      'Sen vertaista toista en mistään ma saa,',
      "on armain ja kallein mull' ain' Suomenmaa.",
      'Ja kiitosta sen laulu soi Sylvian',
      'ja soi aina lauluista sointuisimman.'
    ],
    title: 'Sylvian joululaulu'
  },
  {
    metadata: ['(Lauluosan säv. Piiri pieni pyörii)'],
    number: '501',
    lyrics: [
      '_Runonlausunta sooloesityksenä:_',
      'Monta päivää juotuaan; pessimisti tuskissaan,',
      "tuumi: 'Jo on, jumaliste, saavutettu päätepiste!'",
      'Nuoranpätkä kourassaan; nurkkaan hiipi kuolemaan.',
      'Mutta toiset nähtyään; yhden poissa pöydästään,',
      'sydämissänsä suuri hätä; alkoivat etsiskellä tätä.',
      'Löytyi poika nuorastaan; vainajana suorastaan!',
      'Mutta:',
      '_Lauluosuus, kaikki laulavat:_',
      'Saatuansa spriitä, virkistyi hän siitä.',
      "Kohottaen lasiaan virkkoi: 'Veljet - asiaan!'"
    ],
    title: 'Eino Leinon juomalaulu'
  },
  {
    metadata: [],
    number: '505',
    lyrics: [
      'Trink, trink Brüderlein trink,',
      "lass' doch die Sorgen zu Haus.",
      'Trink, trink Brüderlein trink,',
      'leere dein Glas mit mir aus.',
      ':,: Meide mein Kummer und meide den Schmerz,',
      'dann ist das Leben ein Scherz. :,:',
      'Trink, trink...',
      ':,: Meide die Weiber und meide das Bier,',
      'dann wird ein Sportsman aus dir. :,:',
      'Trink, trink...',
      ':,:Heirat im Sommer und scheide in März',
      'dann ist das Leben ein Schertz. :,:',
      'Trink, trink...',
      ':,: Kauf dir ein Auto und fahr gegen Baum,',
      'dann ist das Leben ein Traum. :,:'
    ],
    title: 'Trink, trink'
  },
  {
    metadata: [],
    number: '506',
    lyrics: [
      "Kaks' kisälliä kulki maantietä laulellen",
      'ja he laulussaan toi julki ylevän aattehen:',
      "'Jos kaikki Suomen järvet viinaksi muuttuisi,",
      'niin eikös meidän poikain (tyttöin) elellä kelpaisi?',
      "Kyllä vaan!'",
      ':,:Kaljaa, kaljaa, kal-jal-lal-lal-lal-laa!:,:',
      "Rannalle Viinajärven majamme rakentais'",
      "ja sen sulolainehilla öin päivin soudeltais'.",
      "Ja me ryypättäis' vain viinaa, viinassa uitaisiin.",
      'Ja eikös tätä voisi verrata Edeniin?',
      'Kyllä vaan!',
      ':,:Kaljaa, kaljaa, kal-jal-lal-lal-lal-laa!:,:',
      'Jos viimein parannusta ei saatais krapulaan,',
      'niin helmaan Viinajärven loikattais kuolemaan.',
      "Ja me ryypättäis' vain viinaa, viinassa uitaisiin.",
      'Ja eikös tätä voisi verrata Edeniin?',
      'Kyllä vaan!',
      ':,:Kaljaa, kaljaa, kal-jal-lal-lal-lal-laa!:,:'
    ],
    title: 'Kaks kisälliä'
  },
  {
    metadata: [
      'Tähän riemastuttavaan pikku lauluun voi aina keksiä lisää säkeitä tarpeen vaatiessa.'
    ],
    number: '509',
    lyrics: [
      'Ken on syntynyt tammikuussa, ylös nouskohon!',
      'Hän tuopin käteen ottakoon ja huulillensa nostakoon.',
      '>>Juo pois, juo pois, juo pois, juo pois,',
      '>>juo Herran tähden pois! (Juo pois!)',
      'Ken on syntynyt helmikuussa...',
      '_Ja samaan syssyyn muutkin kuukaudet_',
      'Ken on syntynyt vahingossa...',
      'Ken on syntynyt tekokuussa...',
      'Ken on syntynyt pullo suussa...',
      'Ken on syntynyt humalassa...',
      'Ken on syntynyt etuperin...',
      'Ken on syntynyt takaperin...',
      'Ken on syntynyt ylipäänsä...'
    ],
    title: 'Ken on syntynyt'
  },
  {
    metadata: [
      'Lauluproseduuri suoritetaan eräajona suorituslokaatioiden varioituessa satunnaisesti (kuitenkin etukäteen syötettyjä ehtolausekkeita noudattaen)'
    ],
    number: '512',
    lyrics: [
      '__KYLMÄ:__',
      '_RUOTSIKSI_',
      'Punschen kommer, punschen kommer,',
      'ljuv och sval.',
      'Glasen immar, rösten stimma,',
      'i vår sal.',
      'Skål för glada minnen!',
      'Skål för varje vår!',
      'Inga sorger finnas mer',
      'när punsch vi får!',
      '_SUOMEKSI_',
      'Punssi saapuu, punssi saapuu, vilpoinen,',
      'lasit kiiltää, ääneet kiitää kilvaten.',
      'Terve joka muisto, terve kaikille.',
      'Huolet ovat haihtuneet, siis maljanne!',
      'Punssi saapuu, punssi saapuu, vilpoinen,',
      'punssi saapuu, punssi saapuu, kultainen!',
      'Malja kevään uuden, malja muistojen!',
      'Punssin taika uudelleen luo nuoruuden!',
      '__LÄMMIN:__',
      '_RUOTSIKSI_',
      'Punschen kommer, punschen kommer,',
      'god och varm.',
      'Vettet svinner, droppen rinner,',
      'ner i tarm.',
      'Skål för alla minnen!',
      'Dem vi snart ej ha,',
      'då ett par glas simmig punsch',
      'vi hunnit ta!',
      '_SUOMEKSI_',
      'Punssi saapuu, punssi saapuu, lämpöinen,',
      'taju haipuu, suoleen taipuu, pisarainen.',
      "Malja muistoillemme, joit' vain muutama",
      "jäljell' on, kun nautittu on lasi muutama."
    ],
    title: 'Punschen kommer - Punssi saapuu'
  },
  {
    metadata: [],
    number: '515',
    lyrics: [
      '__SUOMI:__',
      "Minne? Muisti hävis', mut' minne?",
      'Juhlista selvisimme -',
      'muistikatkoja on.',
      'Minne, lähtisin vaikka minne,',
      'kunhan selvittäsimme,',
      'mitä tapahtunut on.',
      'Mutta tiedän mä keinon,',
      'mikä auttaapi tuo:',
      'Ota ryyppy ja muistis juo!',
      '__RUOTSI:__',
      'Minne! Jag har tappat mitt minne!',
      'Är jag svensk eller finne?',
      '- Kommer inte ihåg...',
      'Inne! Är jag ut eller inne?',
      'Jag har luckor i minnet',
      "- Sån' där små alkohol",
      'Men besinn er, man tätar med det',
      'brännvin man får',
      'medan minnet och helan går.'
    ],
    title: 'Minne - Minnet'
  },
  {
    metadata: ['(Säv. Silja Linen mainostunnari)'],
    number: '517',
    lyrics: [
      '_Rauhallisesti, Sivistyneesti:_',
      'Tahtoisin Gini-Tonicin, Gini-Tonicin.',
      'Tai sitten Minttu-Fernetin, Minttu-Fernetin.',
      'Kelpais myös Kossu-Vissykin, Kossu-Vissykin.',
      'Nyt juhlitaan!',
      '_Reippaasti, Reihakkaasti:_',
      'Tahtoisin Blue Curacaon, Blue Curacaon.',
      'Tai sitten Remy Martinin, Remy Martinin.',
      'Kelpais myös viski-Colakin, viski-Colakin.',
      'Nyt ryypätään!',
      '_Shekaishin, sösshötthäen:_',
      'Tahtoishin raakaa pirtua, raakaa pirtua.',
      'Tai shitten lämmintä oluttha, lämmintä oluttha.',
      'Kelpais myös Masinolikin, Masinolikin.',
      'Nyt oksennan!'
    ],
    title: 'Siljan juomalaulu'
  },
  {
    metadata: ['(Säv. Kisällit kesällä)', '(Säv. Stenka rasin)'],
    number: '522',
    lyrics: [
      'Sörnai gusha nietu Molotova,',
      'sörnai gusha herba Moskova.',
      ':,: Njet, njet bonimai votvot risubushka,',
      'dara zevo votvot harashoo! :,:',
      'Isä Stalin, äiti Crutzheva,',
      "iski silmää gini-hibrakas.'",
      ':,: Oi-oi oisiba vodkaa ollut heillä,',
      'oisi isku käynyt alemmas! :,:',
      'Tuoksuu tuomet, syreenit ja ruusut,',
      'tuoksuu tuolla Volgan randamill.',
      ':,: Ai, ai, kaunis kulda Kaditzusha,',
      'tuoksuu tundeet rindas rindamill! :,:',
      'Siellä, missä versoaabi vilja,',
      'siellä kashvoi gaunis Katjushka.',
      ':,: Katjushkalla komiat on geuhkot,',
      'basga haisee Nevan rannalla! :,:',
      'Oon vain köyhä kolhooshinainen,',
      'ei oo mulla yhtään ystävää.',
      ':,: Ei ole lehmää eikä ole lammasta,',
      'eikä suussa yhtään hammasta! :,:',
      'Siperian lakeus on laaja.',
      'Sonja siellä lunta lapioi. (Hei!)',
      ':,: Sonjalla kävi vitun huono tuuri:',
      'länsituuli uutta lunta toi! :,: (Hei!)',
      'Olen köyhä poika Petroskoista,',
      'ei oo mulla yhtään ystävää.',
      ':,: Sirppi ja vasara ne taivahalla loistaa,',
      'basga haisee, balalaikka soi! :,:',
      'Oon vain pieni tyttö Tshernobylin,',
      'ei oo mulla yhtään ystävää.',
      ':,: Kakstoista varvasta, kolme jalkaa,',
      'kolme silmää, vitun iso pää :,:',
      '_Riskisäveliä:_',
      'Oon vain Riski vapun tietämissä, ',
      'ei oo mulla yhtään krapula.',
      ':,: Korjaussarjaks sain kolme kaljaa,',
      'Jallua ja kylmän kebabin. :,:',
      'Oon vain Iiro Asteriskista,',
      'ei oo mulla yhtään ystävää.',
      ':,: PJ-kaudesta jäljellä vain vatsa,',
      '_Jallupullo sitä lämmittää. :,:_'
    ],
    title: 'Sörnai gusha'
  },
  {
    metadata: ["(Säv. Glada änkan - Vait' on huulet)"],
    number: '510',
    lyrics: [
      'Ken on jäsen Asteriskin, ylös nouskohon!',
      'Hän tuopin käteen ottakoon ja huulillensa nostakoon.',
      'Juo pois, juo pois, juo pois, juo pois,',
      'juo Herran tähden pois! (Juo pois!)',
      'Ken on jäsen Digit ry:n...',
      'jne...'
    ],
    title: 'Ken on jäsen'
  },
  {
    metadata: [
      '(Säv. Jaakko-kulta)',
      '(Tunnetaan myös nimellä Olut-kaanon)',
      'In Memoriam Aura Aboensis'
    ],
    number: '513',
    lyrics: [
      'Lapin Kulta, Lapin Kulta,',
      'Karjala, Karjala.',
      'Sininen ja Aura, Sininen ja Aura.',
      'Koff, Koff, Koff.',
      'Koff, Koff, Koff.',
      'Lauletaan vapaavalintainen määrä säkeitä'
    ],
    title: 'Lapin Kulta'
  },
  {
    metadata: ['(SÄV. LÄHTEELLÄ)', 'Toista täydellistä mielivaltaa käyttäen'],
    number: '514',
    lyrics: ['Jo lähti pois pakenemaan', '- hyvästi varjoinen!'],
    title: 'Runebergin Juomalaulu'
  },
  {
    metadata: ['(Säv. Memories)'],
    number: '511',
    lyrics: [
      'Jos täytätte mun lasini, niin tahdon kertoa',
      "surullisen tarinan, joll' ei oo vertoa.",
      'Se on laulu merirosvosta Roope nimeltään,',
      'joka sydämiä särki, missä joutui kiertämään.',
      'Hän kaunis oli kasvoiltaan ja nuori iältään',
      'ja opetuksen saanut oli omalta isältään.',
      "Mutt' tyttö, jota lempi, hän oli petturi,",
      'ja siksi tuli Roopesta nyt julma ryöväri.',
      "Viel' Itämeren rannikot ne Roopen muistavat,",
      'ja naiset Pietarinkin vielä päätään puistavat.',
      "Ja Suomenlahden rannikot tuns' Roope-ryövärin,",
      'ja pelko oli suuri vielä Kokkolassakin.',
      'Ei neito Viron rannikon unhoita milloinkaan,',
      'kun rosvo-Roopen kanssa hän joutui kapakkaan.',
      'He söivät, mitä saivat ja joivat tuutingin,',
      "ja tyttö poltti sydämens', mutt' Roope sikarin.",
      "Ei tiennyt impi Oolannin, kuink' oli laitansa,",
      "kun rosvo-Roope pestäväks' vei hälle paitansa.",
      "Yks' nappi oli irti, hän ompeli sen kiin'",
      "ja samalla hän ompeli myös sydämensä siin'.",
      'Sai Roope viimein palkkansa, hän on nyt Suomessa',
      'ja jossain jokivarressa lie lossivahtina.',
      'Hän lesken eessä nöyrtyi ja joutui naimisiin',
      'ja sillä lailla rosvo-Roope hiljaa hirtettiin...'
    ],
    title: 'Rosvo-Roope'
  },
  {
    metadata: [
      'Wanhan Asteriski-hallituksen perinteinen bravuurinumero',
      '_Lauletaan reippaasti seisaaltaan, minkä jälkeen juodaan lasi tyhjäksi_',
      '(Säv. trad.)'
    ],
    number: '516',
    lyrics: ['HIP!!!'],
    title: 'Suomalainen juomalaulu'
  },
  {
    metadata: ['(Säv. trad.)'],
    number: '518',
    lyrics: [
      'Jos kaikki Suomen järvet viinaksi muuttuisi',
      'niin eikös meidän poikain elellä kelpaisi.',
      'Ja me rypättäisiin viinaa viinassa uitaisiin,',
      'ja eikös sitä voisi verrata Eedeniin.'
    ],
    title: 'Jos kaikki Suomen järvet'
  },
  {
    metadata: ['(Säv. Joulu on taas)', '(Myös Tänään juodaan) '],
    number: '520',
    lyrics: [
      ':,: Tänään otetaan, tänään otetaan',
      'helevetin paljon viinaa! :,:',
      ':,: Huomenna on, huomenna on',
      'helevetin kova krapula. :,:',
      ':,: I dag ska vi ta, i dag ska vi ta',
      'helvetes mycket brännvin! :,:',
      ':,: I morgon ska vi ha, i morgon ska vi ha',
      'helvetin kova krapula! :,:',
      ':,: Tänä võtame, tänä võtame',
      'kuradima palju viina. :,:',
      ':,: Homme meil on, homme meil on',
      'kuradima kova pohmakas. :,:'
    ],
    title: 'Tänään otetaan'
  },
  {
    metadata: ['(Säv. Täällä pohjantähden alla)'],
    number: '508',
    lyrics: [
      'Täällä jallutähden alla',
      'korkeimmalla kukkulalla.',
      'Katson läpi lasin tyhjän,',
      'sen täytän uudestaan.',
      'Täällä jallutähden alla,',
      'lasi täyttyy leikatulla.',
      'Siitä suojakseni peiton,',
      'minä itselleni saan.',
      '__KERTO__',
      '>>Ja alla jallutähden,',
      '>>minä otan,',
      '>>yhden tähden,',
      '>>ja vain jallutähden nähden,',
      '>>itken ilon kyyneleen.',
      'Täällä jallutähden alla,',
      'kova jano laulajalla.',
      'Huolet viinaa naukkaamalla',
      'siirtyy päivään huomiseen',
      'Täällä jallutähden alla,',
      'hiipii sieluun asti halla,',
      'mutta korkin avaamalla,',
      'sulaa sydän uudelleen.',
      'KERTO'
    ],
    title: 'Alla Jallutähden'
  },
  {
    metadata: ['(Esit. Tapio Rautavaara)'],
    number: '602',
    lyrics: [
      'Sä kasvoit neito kaunoinen isäsi majassa.',
      ':,: Kuin kukka kaunis suloinen,',
      'vihreellä nurmella. :,:',
      'Lapsuuden ajan hellimmän, sä leikit kanssani.',
      ':,: Ja olit ainoo paras ystäväin',
      'ja ainoo iloni. :,:',
      'Maailma sitten vieroitti pois meidät toisistaan.',
      ':,: Vaan sua, armas, iäti,',
      'mä muistan ainiaan. :,:'
    ],
    title: 'Lapsuuden toverille'
  },
  {
    metadata: [],
    number: '521',
    lyrics: [
      ':,: Ja me juomme tämän talon onneksi,',
      'kippis, kippis. :,:',
      ':,: Täällä oleville, poissa oleville',
      'ja vastakin talohon tuleville,',
      'kippis, kippis. :,:',
      ':,: Ja me juomme tämän talon konkurssiin,',
      'kippis, kippis. :,:',
      ':,: Täällä oleville, poissa oleville',
      'ja vastakin talohon tuleville,',
      'kippis, kippis. :,:',
      ':,: Ja me juomme tämän auton bensiinin,',
      'kippis, kippis. :,:',
      ':,: Kaiken öljynkin, koko voitelun',
      'ja helavita akkumalaattorin,',
      'kippis, kippis. :,:'
    ],
    title: 'Tämän talon onneksi'
  },
  {
    metadata: ['(Säv. Stenka Rasin)'],
    number: '523',
    lyrics: [
      'Katon rajas kulkee hauki,',
      'pahaa taitaa tarkoittaa. ',
      ':,: Kierrä pullon korkki auki,',
      'ryyppy pedon karkottaa. :,:',
      'Pöydän alta ryömii lisko,',
      'ilkein silmin tuijottaa. ',
      ':,: Ryyppy toinen naamaan kisko,',
      'aistiharha lähdön saa. :,:',
      'Vierustuolil istuu hylje,',
      'pyrstö läpsii palleaas.',
      ':,: Kunhan vain et lasiin sylje,',
      'mursu daamiks muuttuu taas. :,:',
      'Pöydän päällä ammuu nauta,',
      'maitoo koitta tyrkyttää.',
      ':,: Neljäs snapsi, jumalauta,',
      'maitohapot myrkyttää. :,:'
    ],
    title: 'Hauki ja viina'
  },
  {
    metadata: ['(Säv. Pienet sammakot)'],
    number: '507',
    lyrics: [
      ':,: Pienet konjakit, pienet konjakit, ne lystikkäitä on. :,:',
      ':,: Ei korvia, ei korvia, ei häntää laisinkaan! :,:',
      ':,: Kvak-kvak-kvak-kvak, kvak-kvak-kvak-kvak,',
      'kvak-kvak-kvak-kvak-kvak-kvak! :,:',
      ':,: Kvak-kvak-kvak-kvak, kvak-kvak-kvak-kvak,',
      '_kvak-kvak-kvak-kvak-kvak-kvak! :,:_'
    ],
    title: 'Pienet konjakit'
  },
  {
    metadata: [],
    number: '503',
    lyrics: [
      ':,: Lähtekäämme karhumetsään Reisijärven rantaan. :,:',
      ":,:Siell' on karhu kontallansa, joka ampua antaa. :,:",
      ":,:Vaikk' on karhu karvainenkin, metsämies ei pelkää. :,:",
      ':,:Pistää piipun karhun suuhun ja haulipussit sen selkään. :,:',
      ':,:Vaan ei pyssy lauennutkaan, karhun suu oli märkä. :,:',
      ":,:Siell' on käynyt Käymäläinen ja hässinyt kuin härkä. :,:",
      ':,:Talitintti on pieni lintu, pienempi kuin käki. :,:',
      ':,:Heti käen lempi syttyi, kun tintin pimpin näki. :,:',
      ':,:Käki kuksii kuusikossa pientä tinttiraukkaa. :,:',
      ":,:Vaik' on tintti pieni lintu, niin alta koittaa auttaa. :,:",
      ':,:Pienet linnut kuusikossa toisiansa naivat. :,:',
      ':,:Pois ne lentää liihottaapi, tarpeeksensa kun saivat. :,:'
    ],
    title: 'Karhumetsään '
  },
  {
    metadata: ['(Säv. Volgan lautturi)'],
    number: '519',
    lyrics: [
      'Lisää viinaa. (Ho!)',
      'Silmät liikkuu. (Ho!)',
      'Lisää viinaa, silmät liikkuu.',
      'Repeat Ad Infinitum',
      '6 Vaaleanpunaiset'
    ],
    title: 'Lissää viinaa, silmät liikkuu'
  },
  {
    metadata: ['(Esit. mm. Mauno kuusisto)', '(Myös Kossun ylistys)'],
    number: '601',
    lyrics: [
      ":,: Minun kultani kaunis on, vaikk' on kaitaluinen. :,:",
      ":,: Hei luulia illallaa, vaikk' on kaitaluinen. :,:",
      ":,: Kullan pää on pähkinä, vaikk' on väärällänsä. :,:",
      ":,: Hei luulia illallaa, vaikk' on väärällänsä. :,:",
      ":,: Tukka on tummanruskea, vaikk' on tappurainen. :,:",
      ":,: Hei luulia illallaa, vaikk' on tappurainen. :,:",
      ":,: Silmät sill' on siniset, vaikk' on kieronlaiset. :,:",
      ":,: Hei luulia illallaa, vaikk' on kieronlaiset. :,:",
      ':,: Suu on sillä supukka, vaikkä on toista syltä. :,:',
      ":,: Hei luulia illallaa, vaikk' on toista syltä. :,:",
      ':,: Kun minä vien sen markkinoille, niin hevosetkin nauraa. :,:',
      ':,: Hei luulia illallaa, hevosetkin nauraa. :,:'
    ],
    title: 'Kullan ylistys'
  },
  {
    metadata: [],
    number: '603',
    lyrics: [
      ':,: Niin minä, neitonen, sinulle laulan kuin omalle kullalleni :,:',
      ':,: Jos olis valta, niin kuin on mieli, niin ottaisin omakseni :,:',
      ':,: Kullalleni minä laulelen ja kellekäs sitten muille :,:',
      ':,: En minä laula kallioille, enkä metsän puille :,:'
    ],
    title: 'Niin minä neitonen'
  },
  {
    metadata: ['(Esit. Leevi and the Leavings)'],
    number: '605',
    lyrics: [
      '__KERTO:__',
      '>>Minä sinulle tehdä jos saisin, sitä mitä',
      '>>silloin en kehdannut kysyäkään.',
      '>>Rinnoista sua puristaisin, kun ei ne',
      '>>piilossa tahtoneet pysyäkään.',
      '>>Sä olit sopivasti lihava.',
      'Sinä typerä olla saat,',
      'kanamainen hölmä nainen.',
      'Sun turha on luulla',
      'mun tahtovan kuulla jotain järkevää.',
      'Ne sinun muotosi kurvikkaat',
      'saivat aikaan valtavan paineen,',
      'kun päällesi puskin,',
      'niin muu oli tuskin yhtä tärkeää.',
      '__KERTO__',
      'En edes tiedä, miksi kiihottaa',
      'mua yhtä pitkä kuin leveä nainen.',
      'Jalat kuin pylväät ja',
      'rinnat niin ylväät, joista kiinni saa.',
      'Se nainen unissani liihottaa,',
      'se lähes sadan kilon keijukainen.',
      'Muodokas kyllä,',
      'kun ei vaatteita yllä ole ollenkaan.',
      'KERTO'
    ],
    title: 'Sopivasti lihava'
  },
  {
    metadata: [],
    number: '606',
    lyrics: [
      'Sua vain, yli kaiken mä rakastan,',
      'sinä taivaani päällä maan.',
      'Minä maiset murheeni unohdan,',
      'sinisilmiis kun katsoa saan.',
      'Yli riemuni oot mitä toivoisin',
      'sä kun istut mun rinnallain',
      'Olet kaikkeni, aarteeni, kallehin',
      'Olet onneni unelmain.',
      'Oi, sallithan, että mä suutelen',
      'sinisilmiäs, armahain.',
      'Ja jos kauas sun luotasi joutunen',
      'kuvas seuraa mun matkallain.',
      'Ja jos kauas sun luotasi joutunen',
      'kuvas seuraa mun matkallain.',
      'Yli riemuni oot mitä toivoisin',
      'sä kun istut mun rinnallain.',
      'Olet kaikkeni, aarteeni, kallehin,',
      'olet onneni, unelmain.'
    ],
    title: 'Romanssi'
  },
  {
    metadata: ['(Esit. Neon 2)'],
    number: '608',
    lyrics: [
      'Sen illan puistossa me istuttiin,',
      'yömyöhään juteltiin.',
      'Puiston hämärässä suudeltiin,',
      'viiniä nautittiin.',
      'Sä kerroit kuinka mukavaa',
      'ois kantaa sormusta,',
      'pieni mökki metsän laitaan laittaa,',
      'kukkia kasvattaa.',
      '__KERTO:__',
      'Mut hei ei se riitä, et on mukavaa,',
      'sen täytyy olla paljon parempaa.',
      'Siinä täytyy olla rakkautta,',
      'siinä täytyy olla unelmii,',
      'mannapuuroa ja mansikkaa,',
      'juuri oikeenlaista kemiaa.',
      'Sen täytyy joskus satuttaa,',
      'sen täytyy voida naurattaa.',
      'Ei siitä muuten mitään tuliskaan',
      'ilman oikeenlaista kemiaa',
      'Alkoi katuvalot sammumaan',
      'ja meitä paleltaa.',
      'Sä pyysit mua vierees nukkumaan,',
      'joo, mä olin valmiina.',
      'Aamuyöstä vielä kuuden aikaan',
      'kuuntelin haaveitas,',
      'seinän takaa isäs kuorsausta,',
      'hmm.. mä taisin huokaista.',
      '__KERTO__',
      'Ei elämää liikaa suunnitella saa,',
      'antaa sen mennä vain painollaan.',
      'Jos joskus vielä tavataan - hyvä niin',
      'jos ei - lakit päähän ja näkemiin.',
      'KERTO'
    ],
    title: 'Kemiaa'
  },
  {
    metadata: ['(Esit. Irwin Goodman)'],
    number: '701',
    lyrics: [
      'Kaikki alkoi siitä kun mä lopputilin sain.',
      'Sitten juhlan kunniaksi täyden lastin hain.',
      'Eikä siinä turhaan liioin kainosteltu lain.',
      'Pullo kiersi ringissä ja irvisteltiin vain.',
      '__KERTO:__',
      '>>:,: Ei tippa tapa ja ämpäriin ei huku!',
      '>>Kun on alkuun päästy niin antaa mennä vaan. :,:',
      'Elämä on tylsää jos ei joskus juhli näin.',
      'No kuka sitä aina jaksaa olla selvinpäin.',
      'Antaa mennä samaan tahtiin viikko peräkkäin.',
      'Kaikki murheet huuhdotaan me sisältämme näin.',
      '__KERTO__',
      'Kaikkihan me täällä ollaan pieni hetki vaan.',
      'On aivan turhaa mammonaa näin koota päälle maan.',
      'Joku toinen kuitenkin sen ryyppää aikanaan.',
      'Juodaan kaikki nyt ja juokoon toiset omiaan.',
      '__KERTO__',
      'Juodaan valinnaisia juomia valinnaisissa paikoissa valinnaisilla hetkillä valinnainen määrä'
    ],
    title: 'Ei tippa tapa'
  },
  {
    metadata: ['(Säv. Rosvo-Roope)'],
    number: '702',
    lyrics: [
      'Jos täytätte mun lasini, niin tahdon kertoa',
      'Jos täytätte mun lasini, niin tahdon kertoa',
      'Jos täytätte mun lasini, niin tahdon kertoa',
      'Jos täytätte mun lasini, niin tahdon kertoa',
      'Nyt täynnä on mun lasini, en ehdi kertoa ...',
      'Taas tyhjä on mun lasini, en jaksa kertoa ...',
      'Jos löydätte mun lasini, niin näen kertoa ...',
      'No täytättekös lasini, tai alan kertoa ...'
    ],
    title: 'Jos täytätte mun lasini'
  },
  {
    metadata: ['(Säv. Heili Karjalasta)'],
    number: '705',
    lyrics: [
      'Kauas maailmaan läksin kulkemaan,',
      'kuljin niin iloisna ja huoletonna vain',
      'Jouduin kapakkaan, kuinkas sattuikaan!',
      'Janonpiston kurkkuuni sain',
      '__KERTO:__',
      '>>Hei, kapakasta kaljan minä löysin,',
      '>>löysin kaljan, pitkän huurteisen',
      '>>Se kohta minut sitoi mallasköysin,',
      '>>sitoi humalahan iäisehen',
      '>>Tunne outo kurkussani on kuin tyhjiö vatsassani,',
      '>>ja kuitenkin niin onnellinen oon!',
      '>>Hei, kapakasta kaljan minä löysin,',
      '>>vaivuin humalahan iäisehen',
      'Vaan en saanutkaan tuonne onnelaan',
      'jäädä - täytyi jälleen rientää kauas pois',
      "Mutta rinnassain yks' on aatos vain;",
      'kapakkaan taas jospa päästä vois',
      '__KERTO__',
      'Ja mä tiedän sen, tuoppi huurteinen,',
      'oottaa siellä kapakan tiskillä tyhjentäjää,',
      "jok' ei konsanaan luovu aarteestaan,",
      'ennenkuin vessanpönttöön oksentaa',
      'KERTO'
    ],
    title: 'Kalja kapakasta'
  },
  {
    metadata: ['(Esit. mm. Eero Koivistoinen, yhtye)'],
    number: '607',
    lyrics: [
      'Yksi ruusu on kasvanut laaksossa',
      'ja se kauniisti kukoistaa.',
      'Yksi kulkijapoika on nähnyt sen,',
      'eikä voi sitä unhoittaa.',
      'Ja hän olisi sen kyllä poiminut',
      'ja sen painanut povelleen,',
      'mutta köyhänä ei ole tohtinut',
      'vaan on jättänyt paikoilleen.'
    ],
    title: 'Yksi ruusu on kasvanut laaksossa'
  },
  {
    metadata: ['(Säv. Kun mä kuolen)'],
    number: '604',
    lyrics: [
      ':,:Kun mä nuolen,',
      'multiorgasmin sä saat:,:',
      'Sillä karhee on mun',
      'kieli, perverssi on mulla mieli',
      'Kun mä nuolen',
      'multiorgasmin sä saat!',
      '7 Humalahakuiset',
      'Shiish mitä? Jaaaaa... Nää on näith, thieräkshä, juomalauluja. Burp! Hupsh, nyt tulee "haukotush"...'
    ],
    title: 'Multiorgasmi'
  },
  {
    metadata: ['(Säv. trad.)'],
    number: '703',
    lyrics: ['Ny!!!'],
    title: 'Juomalaulu ei-laulajille (lyhyt versio)'
  },
  {
    metadata: ['(Säv. trad.)'],
    number: '704',
    lyrics: [
      '__SUOMI:__',
      'Ei ny',
      'Vaan ny!',
      '__RUOTSI:__',
      "Int' nu",
      'Men nu!'
    ],
    title: 'Juomalaulu ei-laulajille (pitkä versio)'
  },
  {
    metadata: ['(Säv. Mörri-möykky)'],
    number: '706',
    lyrics: [
      'Ottaisin minä Karhu-viinaa, jos vain jostain saisin',
      ':,: Koko pullon ryyppäisin ja sitten oksentaisin :,:',
      '>>Tiu-tau-tiu-tau-tili-tali-tittan,',
      '>>juodaan rommia kolalla',
      '>>:,: Sitten kun rommikolat on juotu,',
      '>>niin ollaan perseet olalla :,:',
      'Vaan eipä taida meidän äiti Karhu-viinaa ottaa,',
      ':,: eihän se edes usko, että Karhu-viina on totta :,:',
      '>>Tiu-tau-tiu-tau-tili-tali-tittan,',
      '>>juodaan kossua vissyllä',
      '>>:,: Sitten kun kossuvissyt on juotu,',
      '>>niin ollaan perseet olalla :,:'
    ],
    title: 'Karhu-viinaa'
  },
  {
    metadata: ['(Säv. Kullan ylistys)'],
    number: '707',
    lyrics: [
      ":,: Minun juomani Kossu on, vaikk' on viljaviinaa :,:",
      ":,: Hei luulia illallaa, vaikk' on viljaviinaa :,:",
      ':,: Silmät siit saan punaiset sekä kieronlaiset :,:',
      ':,: Hei luulia illalla sekä kieronlaiset :,:',
      ':,: Kun minä juon sitä markkinoilla niin hevosetkin nauraa :,:',
      'Hei luulia illalla hevosetkin nauraa',
      'Hei luulia hah-hah-haa! - hevosetkin nauraa',
      ':,: Kun minä menen kapakkaan, niin portsaritkin nauraa :,:',
      ':,: Hei luulia illalla portsaritkin nauraa :,:',
      ":,: Mull' on räkä poskella sekä turpa täynnä :,:",
      ':,: Hei luulia illalla sekä turpa täynnä :,:'
    ],
    title: 'Kossun ylistys'
  },
  {
    metadata: ['(Säv. trad.)'],
    number: '708',
    lyrics: [
      'On kotikalja mulla tuolla',
      'peräkammarissa, siellä juonut olen',
      'viikon tai kaksi, siksi varmaan',
      'mulla nenä punoittaa',
      'Kun suviaika vihdoin koittaa',
      'peräkammarissa, siellä soolon soittaa',
      'kilju tuo käymistilassansa',
      'Tuskin maltan odottaa!',
      '__KERTO:__',
      '>>Sinne vielä tahdon mennä,',
      '>>siellä on kaikki rempallaan',
      '>>Isä, äiti, siskot, veljet,',
      '>>mummokin perseet olallaan',
      'Kankkunen ei mua koskaan vaivaa,',
      'kotikaljalla sen aina tieltä raivaan',
      'Kun ilta ehtii, voin mä taivaan',
      'avulla sen saavuttaa',
      'Näin vedän kevään ensi kännin,',
      'toisin sanoen kahden kuukauden rännin',
      'Syksyllä sitten hilpeämmin',
      'uusi pönttö pulputtaa',
      '__KERTO__',
      '__EKA SÄE UUSIKSI__',
      '__KERTO__'
    ],
    title: 'Kotikalja'
  },
  {
    metadata: [],
    number: '715',
    lyrics: [
      'Me tulimme kapakasta',
      'ja me menimme kapakkaan',
      'Me tulimme kapakasta',
      'ja me menimme kapakkaan',
      'Me tulimme kapakasta',
      'ja me menimme kapakkaan',
      'Me tulimme kapakasta',
      'ja me menimme kapakkaan',
      '...',
      '_Toista_'
    ],
    title: 'Kuubalainen serenadi (humala-mix)'
  },
  {
    metadata: ['(Säv. Karjalan poikia)', '(Myös Silimät suuret ja Sibboborna)'],
    number: '709',
    lyrics: [
      'Viina on elämän riemuja varten,',
      'viina on lahjana jumalatarten',
      '__KERTO:__',
      '>>Leukojen välliin, leukojen välliin,',
      '>>leukojen välliin viinaa, juu!',
      '>>Leukojen välliin ryyppi,',
      '>>nyt joka ainut tyyppi!',
      '>>Leukojen välliin, leukojen välliin,',
      '>>leukojen välliin viinaa, juu!',
      'Ryypillä tehdään kuninkaita,',
      'herroja suuria ruhtinaita',
      '__KERTO__',
      'Ryyppi se painuu sisuksihin syvvää,',
      'päässä ja syämmessä tekkee se hyvvää',
      '__KERTO__',
      'Ryyppi se uppoo miehiin ja frouviin,',
      'kaikki sen saa, jotka tulleet on krouviin',
      '__KERTO__',
      'Ryyppi kun mennyt on kurkusta sissää,',
      'siellä se istuu ja pyytää lissää',
      'KERTO'
    ],
    title: 'Leukojen välliin'
  },
  {
    metadata: ['(Säv. Mikki-hiiri ja susihukka)', '(Myös Öykky-möykky)'],
    number: '710',
    lyrics: [
      'Kas Mikki-hiiri mättähältä mättähälle käyt',
      'ja pikkujalka vilkahtaa',
      "Vaan viis me tuosta Mikki-hiirest', häntä kun ei näy",
      '- siis jatkakaamme iltaa vaan!',
      '>>Niukka naukku, niukka naukku,',
      '>>nyt mä pistän sut poskehen!',
      '>>Tiukka tuikku, tiukka tuikku,',
      '>>nyt mun hullusti käy!'
    ],
    title: 'Mikki-Hiiren juomalaulu'
  },
  {
    metadata: ['(Säv. O, tannebaum!)'],
    number: '712',
    lyrics: [
      '__SUOMI:__',
      'Nyt ryypätään, nyt ryypätään,',
      'nyt ryypätään, ryypätään!',
      'Nyt ryypätään, nyt ryypätään,',
      'nyt ryypätään, nyt ryypätään!',
      'Nyt ryypätään, nyt ryypätään,',
      'nyt ryypätään, ryypätään!',
      '__OTETAAN RYYPPY__',
      'Nyt ryypättiin, nyt ryypättiin,',
      'nyt ryypättiin, ryypättiin.',
      'Nyt ryypättiin, nyt ryypättiin,',
      'nyt ryypättiin, nyt ryypättiin.',
      'Nyt ryypättiin, nyt ryypättiin,',
      'nyt ryypättiin, ryypättiin.',
      '__RUOTSI:__',
      'Nu tar vi den, nu tar vi den,',
      'nu tar vi den, nu tar vi den...',
      '__EN RYYPPY TAGAS__',
      'Nu tog vi den, nu tog vi den,',
      'nu tog vi den, nu tog vi den...',
      '__LISÄSÄKEET SUOMEKSI:__',
      'Voi Tampere, voi Helsinki,',
      'voi Lappeenranta, Turku!...',
      'Voi saatana, voi perkele,',
      'voi jumalauta vittu!...'
    ],
    title: 'Nyt ryypätään - Nu tar vi den'
  },
  {
    metadata: ['(Esit. Monty Python)'],
    number: '713',
    lyrics: [
      'Immanuel Kant was a real piss-ant',
      'who was very rarely stable',
      'Heidegger, Heidegger was a very boozy beggar',
      'who could think you under the table',
      'David Hume could outconsume',
      'both Schopenhauer and Hegel,',
      'and Wittgenstein was a beery swine',
      'who was just as schlosched as Schegel',
      ">>There's nothing Nietzsche couldn't teach you",
      ">>'bout the raising of the wrist",
      '>>Socrates himself was permanently pissed',
      'John Stuart Mill, of his own free will,',
      'on half a bottle of Shandy was particulary ill',
      'Plato, they say, could stick it away',
      'half a crate of whisky every day',
      'Aristotle, Aristotle was a bugger for the bottle,',
      'Hobbes was fond of his dream',
      'And Rene Descaretes was a drunken fart:',
      "'I drink, therefore I am'",
      '>>Yes, old Socrates himself',
      '>>is particuraly missed,',
      ">>a lovely little thinker but a bugger when he's pissed"
    ],
    title: "Philosopher's Song"
  },
  {
    metadata: ['(Säv. Raumalainen kansanlaulu)'],
    number: '719',
    lyrics: [
      'Kaad liiveihis ryypp,',
      'hyväs seoras vaik kaks,',
      'kaad kolmaski viäl',
      'jos snuu veres on baks!',
      'Älä ussemppa kaad',
      'ann menn siut suu,',
      'sill muutoin käy nii',
      'et se ryypp kaata snuu!'
    ],
    title: 'Raumlase ryypp'
  },
  {
    metadata: [
      '(Säv. Karjalan poikia)',
      '(Myös Leukojen välliin ja Silimät suuret)'
    ],
    number: '714',
    lyrics: [
      'Brännvini bränner och bultar och dunkar,',
      'sibboborna huttar och klunkar:',
      'Tar sig en hela, tar sig en halva',
      'och tersen och kvarten och kvinten på',
      '>>Tar dem på allvar liksom ystra kalvar,',
      '>>tar sig en hela, tar sig en halva',
      '>>och tersen och kvarten och kvinten på!',
      'En del har frågat och somliga undrat',
      'kan man ta starkt när det blixtrar och dudrar?',
      'Kan man ta helan, kan man ta halvan',
      'och tersen och kvarten och kvinten på?',
      '>>Svar till er alla: Skåla när det knallar',
      '>>Pang - där gick helan! Pang - där gick halvan!',
      '>>Och tersen och kvarten och kvinten på!'
    ],
    title: 'Sibboborna'
  },
  {
    metadata: [],
    number: '716',
    lyrics: [
      'Ukko Nooa, Ukko Nooa oli kunnon mies',
      "Kun hän jätti arkin luo Araratin 'parkin',",
      "istutti hän viinitarhan, suuren kuka ties'",
      'Nooa souti, Nooa souti kauas maailmaan',
      'Lievittääkseen piinaa hän hankki pullon viinaa',
      'sammuttaakseen, sammuttaakseen kovaa janoaan',
      'Noo tiesi, Nooa tiesi, että ihminen',
      'janoinen on aina varsinkin lauantaina',
      'Siksi viinin istutti hän runsassatoisen',
      'Nooan vaimo, Nooan vaimo kunniallinen',
      'antoi pullon tuoda, siitä Nooan juoda',
      'Jospa myöskin vaimo minun olis sellainen!',
      'Koskaan ei hän kieltänytkään Nooa juomasta',
      'Jälkeen kännin moisen jo tahtoi Nooa toisen',
      'savipullostansa vaimon kotiin tuomasta',
      'Ukko Nooa, Ukko Nooa valkotukkainen,',
      'pappa rusoposki, nyt pujopartaan koski',
      'pohjaan juoden, pohjaan juoden maljan hurraten',
      'Hauskaa silloin, hauskaa silloin oli päällä maan',
      'Paukun ensimmäisen sai vieras, lähimmäinen,',
      'joka milloin sattui illoin pöytään istumaan',
      'Huolta ei siis liene ollut olla selvin päin',
      'Tavat unohdettiin, kun viini nousi lettiin',
      'Pohjaan saakka, pohjaan saakka juotiin maljat näin'
    ],
    title: 'Ukko Nooa - Gubben Noach'
  },
  {
    metadata: [],
    number: '720',
    lyrics: [
      '__4X__',
      'Vem i helveten kastar sten',
      'på min flaska brännvin?!',
      '__4X__',
      'Kuka helvetti heitti kiven',
      'mun viinapulloon?!',
      '__4X__',
      'Who the hell threw a rolling stone',
      'to my whisky bottle?',
      '__4X__',
      'Saku Sammakko heitti kiven',
      'mun viinapulloon!',
      '__4X__',
      'Saku Sammakkoo on niin helvetin',
      'helppo syyttää!',
      '__4X__',
      'Kuka helvetti heitti minutkin',
      'viinapulloon?'
    ],
    title: 'Vem i helvete - Kuka helvetti'
  },
  {
    metadata: ['(Säv. Jeepers Creepers)'],
    number: '718',
    lyrics: [
      'Viinaa, vettä',
      '- mitä perkelettä?',
      'Viinaa raakaa',
      'napaan kaatakaa!',
      'Brännvin, vatten,',
      'smakar skit som katten!',
      'Brännvin helt råt',
      'smakar jävligt gott!'
    ],
    title: 'Viinaa, vettä - Brännvin, vatten'
  },
  {
    metadata: [],
    number: '717',
    lyrics: [
      'What shall we do with the drunken sailor? | x3',
      'Early in the morning.',
      '__KERTO:__',
      '>>Hooray and up she rises,',
      '>>hooray and up she rises.',
      '>>Hooray and up she rises,',
      '>>early in the morning.',
      "Put him in the longboat 'till he's sober. | x3",
      'Early in the morning.',
      '__KERTO__',
      "Put him in bed with the captain's daughter. | x3",
      'Early in the morning.',
      '__KERTO__',
      'Put him in the scuppers with a hosepipe on him. | x3',
      'Early in the morning.',
      '__KERTO__',
      'Pull out the plug and wet him all over. | x3',
      'Early in the morning.',
      '__KERTO__',
      'Heave him by the leg in a running bowline. | x3',
      'Early in the morning.',
      '__KERTO__',
      'Keelhaul him, keelhaul him. | x3',
      'Early in the morning.',
      '__KERTO__',
      'Shave his belly with a rusty razor. | x3',
      'Early in the morning.',
      '__KERTO__',
      "That's what we'll do with a drunken sailor. | x3",
      'Early in the morning.',
      'KERTO'
    ],
    title: 'What Shall We Do With the Drunken Sailor?'
  },
  {
    metadata: ['(Säv. Mannakorven mailla)'],
    number: '801',
    lyrics: [
      'Levanteri kysy sitä panemisen hintaa,',
      'emäntä se siveli vain pimppinsä pintaa',
      '__KERTO:__',
      '>>Hura-huh-hah-hei, ei nähty näillä mailla',
      '>>ole Levanterin kullia komiampaa!',
      "Levanteri kysy: 'Paljon naiminen maksaa?'",
      "Emäntä se sano, että: 'Kymppi on taksa!'",
      '__KERTO__',
      "Emäntä se sano, että: 'Jos kymppi on liikaa,",
      "niin mene sinä navettaan ja nai karjapiikaa!'",
      '__KERTO__',
      'Isäntä se sängyn alla kyttäsi ja vahti,',
      'mikä oli Levanterin panemisen tahti',
      '__KERTO__',
      'Levanteri ulkosalla hyppäsi ja loikki,',
      'paita oli veressä ja kyrpä oli poikki',
      '>>Hura-huh-hah-hei, ei nähdä näillä mailla',
      '>>enää Levanterin kullia komiampaa...'
    ],
    title: 'Levanteri'
  },
  {
    metadata: [],
    number: '804',
    lyrics: [
      "I know someone you don't know",
      'Yogi, Yogi.',
      "I know someone you don't know",
      'Yogi, Yogi bear.',
      'Yogi, Yogi bear,',
      'Yogi, Yogi bear.',
      "I know someone you don't know",
      'Yogi, Yogi bear.',
      'Yogi has a girlfriend',
      'Cindy, Cindy.',
      'Yogi has a girlfriend',
      'Cindy, Cindy bear.',
      'Cindy, Cindy bear,',
      'Cindy, Cindy bear.',
      'Yogi has a girlfriend',
      'Cindy, Cindy bear.',
      'Cindy likes it from behind',
      'from be, from be.',
      'Cindy likes it from behind',
      'from be, from behind.',
      'From be, from behind,',
      'from be, from behind.',
      'Cindy likes it from behind',
      'from be, from behind.',
      'Yogi has a little friend',
      'Bobo, Bobo.',
      'Yogi has a little friend',
      'Bobo, Bobo bear.',
      'Bobo, Bobo bear,',
      'Bobo, Bobo bear.',
      'Yogi has a little friend',
      'Bobo, Bobo bear.',
      'Bobo likes it up the ass',
      'up the, up the.',
      'Bobo likes it up the ass',
      'up the, up the ass.',
      'Up the, up the ass,',
      'up the, up the ass.',
      'Bobo likes it up the ass',
      'up the, up the ass.',
      'Yogi has an enemy',
      'Ranger, Ranger.',
      'Yogi has an enemy',
      'Ranger, Ranger Smith.',
      'Ranger, Ranger Smith,',
      'Ranger, Ranger Smith',
      'Yogi has an enemy',
      'Ranger, Ranger Smith.',
      'Ranger Smith fucks animals',
      'ani, ani.',
      'Ranger Smith fucks animals',
      'ani, animals.',
      'Ani, animals,',
      'ani, animals.',
      'Ranger Smith fucks animals',
      'ani, animals.',
      'Cindy has a girlfriend,',
      'Betsy, Betsy.',
      'Cindy has a girlfriend,',
      'Betsy, Betsy bear.',
      'Betsy, Betsy bear,',
      'Betsy, Betsy bear.',
      'Cindy has a girlfriend,',
      'Betsy, Betsy bear.',
      'Betsy likes it in the group',
      'gangbang, gangbang.',
      'Betsy likes it in the group,',
      'gang-ga gang gangbang.',
      'Gang-ga gang gangbang,',
      'gang-ga gang gangbang.',
      'Betsy likes it in the group',
      'gang-ga gang gangbang.',
      "Yogi's into whips and chains,",
      'kinky, kinky.',
      "Yogi's into whips and chains,",
      'kinky, kinky bear.',
      'Kinky, kinky bear,',
      'kinky, kinky bear.',
      "Yogi's into whips and chains,",
      'kinky, kinky bear.',
      'Yogi likes it in the fridge,',
      'polar, polar.',
      'Yogi likes it in the fridge,',
      'polar, polar bear.',
      'Polar, polar bear,',
      'polar, polar bear.',
      'Yogi likes it in the fridge,',
      'polar, polar bear.',
      'Yogi likes them dead and cold,',
      'necro, necro.',
      'Yogi likes them dead and cold,',
      'necrophilic bear.',
      'Necrophilic bear,',
      'necrophilic bear.',
      'Yogi likes them dead and cold,',
      'necrophilic bear.',
      'Yogi likes it young and sweet,',
      'pedo, pedo.',
      'Yogi likes it young and sweet,',
      'pedo, pedobear.',
      'Pedo, pedobear,',
      'pedo, pedobear.',
      'Yogi likes it young and sweet,',
      'pedo, pedobear.',
      'Yogi has a seven inch,',
      'seven, seven.',
      'Yogi has a seven inch,',
      'seven inch cock.',
      'Seven inch cock,',
      'seven inch cock.',
      'Yogi has a seven inch,',
      'seven inch cock.',
      "Yogi sits on Santa's lap,",
      "feelin', feelin'",
      "Yogi sits on Santa's lap,",
      "feelin' something hard.",
      "Feelin' something hard,",
      "feelin' something hard.",
      "Yogi sits on Santa's lap,",
      "feelin' something hard."
    ],
    title: 'Yogi Bear'
  },
  {
    metadata: ['(Säv. Tonttu-ukot)'],
    number: '711',
    lyrics: [
      'Hei, tenttu-ukot hyppikäätte,',
      "viel' on viinakaupat auki!",
      'Hei, tenttu-ukot hyppikäätte,',
      "viel' on viinakaupat auki!",
      'Vain hetken kestää humalaa,',
      'ja se on helkkarin mukavaa!',
      'Hei, tenttu-ukot hyppikäätte,',
      "viel' on apteekit auki!",
      'Hei, tenttu-ukot hyppikäätte,',
      "viel' on apteekit auki!",
      'Vain hetken kestää humalaa,',
      'ja se on helkkarin mukavaa!'
    ],
    title: 'Tenttu-ukot'
  },
  {
    metadata: [],
    number: '802',
    lyrics: [
      ':,: Saku Sammakko kosiomatkallaan, ahaa, ahaa :,:',
      'Saku Sammakko kosiomatkallaan',
      'veti Hillevi Hiirtä pulkassaan, ahaa, ahaa, ahaa',
      ':,: Saku Sammakko kosiomatkallaan, ahaa, ahaa :,:',
      'Saku Sammakko kosiomatkallaan',
      'teki sitä ihan seisaaltaan, ahaa, ahaa, ahaa',
      ':,: Saku Sammakko veti oikein vauhdilla, ahaa, ahaa :,:',
      'Saku Sammakko veti oikein vauhdilla',
      'nyt Hillevi Hiirtä pulkassaan, ahaa, ahaa, ahaa',
      ':,: Mutta Hillevi ei ollut moksiskaan, ahaa, ahaa :,:',
      'Mutta Hillevi ei ollut moksiskaan,',
      'sillä nainen on nainen aina vaan, ahaa, ahaa, ahaa',
      ':,: Nyt Hillevi Hiirestä huomataan, voi ei, voi ei :,:',
      'Nyt Hillevi Hiirestä huomataan,',
      'että Saku teki sitä paljaaltaan, voi ei, voi ei, voi ei',
      ':,: Sai myös Saku muiston itselleen, aih aih, aih aih :,:',
      'Sai myös Saku muiston itselleen,',
      'satiaiset, kupan ja syfiliksen, aih aih, aih aih, aih aih'
    ],
    title: 'Saku Sammakko (Horny Mix)'
  },
  {
    metadata: [
      '(Säv. Mikki-Hiiri)',
      'Tämä riemastuttava sarjakuvalaulu on alunperin Deltan tekosia,johon * on tehnyt pari lisäsäkeistöä'
    ],
    number: '803',
    lyrics: [
      'Taas Mikki Hiiri mättähältä Minnin päälle käy,',
      "pikku 'jalka' vilkahtaa",
      "Minni huutaa hädissänsä: 'Kumia ei näy!'",
      'johon Mikki virkahtaa:',
      '__KERTO:__',
      '>>Öykky-möykky, öykky-möykky,',
      '>>nyt mä panen sua takaapäin!',
      '>>Öykky-möykky, öykky-möykky,',
      '>>nyt sun hullusti käy!',
      'Taas Aku Ankka lampehelta Iineksen päälle käy,',
      'pikku-Aku vilkahtaa',
      "Iines huutaa hädissänsä: 'Housujas' ei näy!'",
      'johon Aku virkahtaa:',
      '__KERTO__',
      'Taas Hannu Hanhi aitassa Hansun päälle käy,',
      'pikku-Hanski vilkahtaa',
      "Hansu huutaa hädissänsä: 'Mummoa ei näy!'",
      'johon Hannu virkahtaa:',
      'KERTO'
    ],
    title: 'Öykky-Möykky'
  },
  {
    metadata: [],
    number: '808',
    lyrics: [
      'Omistettu kaikille Suomen pikku oraville siellä ulkona',
      'Mä olen pikku Tiku! Mä olen pikku Taku!',
      'On työnämme ja huvinamme pullojen vain haku!',
      'Lala-lallati-lallati-laa! Hauska cityssä on asustaa!',
      'Me kahteen pekkaan perustimme ikioman kerhon,',
      "ja jäsenmaksuks' määräsimme yhden kossupullon",
      'Lala-lallati-lallati-laa, lala-lallati-lallati-laa!',
      'Ja Turun keskustassa on koti meillä, jossa',
      'ei tarkastaja pääse väijymään kuin joskus vahingossa!',
      'Lala-lallati-lallati-laa! Hyllyt notkua pulloista saa!',
      'Ei puutu meiltä konettakaan, pöydällä on PC',
      'Niin tehoton ja vanha, ettei Windowskaan pyöri',
      'Lala-lallati-lallati-laa, lala-lallati-lallati-laa!',
      'On koulu yhtä piinaa, me vedetään vain viinaa!',
      'On oma känni oravankin onnen valtakunta',
      'Lala-lallati-lallati-laa! Ryyppy känniä piristää!',
      'Niin viihtyisää ja riemukasta on heilua humalassa',
      'Ja illan työstä ahkerasta kielii aamun grabbis',
      'Lala-lallati-lallati-laa, Lala-lallati-lallati-laa!'
    ],
    title: 'Tiku ja Taku'
  },
  {
    metadata: ['(Säv. Stenka rasini)', '(Myös Sörnai gusha)'],
    number: '807',
    lyrics: [
      'Liten Jumbo flyger över Sovjet,',
      'Mig 23 går upp och skjuter ner',
      ':,: Liten Jumbo störtar ner i havet,',
      'Liten Jumbo flyger aldrig mer :,:',
      'Liten U-båt simmer i Sverige,',
      'liten U-båt stöter på ett grund',
      ':,: Liten U-båt flyter upp till ytan,',
      'liten U-båt simmer aldrig mer :,:',
      'Liten Ivan springer over stäppen,',
      'K-pist smattra Ivan faller ner',
      ':,: Blod och slamsor spridas over stäppen,',
      'liten Ivan springer aldrig mer :,:',
      'Litet kraftverk smäller uti Sovjet,',
      'ryssar dör som flugor runt i kring',
      ':,: Hela saket tystas ner av staten,',
      'litet kraftverk finns ej längre mer :,:',
      'Liten Olof kommer ut från bion,',
      'Krister skjuter Olof Palme ner',
      ':,: Olof Palme störtar ner till gatan,',
      'Olof palme finns ej längre mer :,:',
      "Liten mig den flyger över Ek'näs,",
      'liten mig den börjar falla ner',
      ':,: Liten mig den störtar ner i Leksvall,',
      'liten mig den flyger aldrig mer :,:',
      'Ilpo Larha sitter i en bostad,',
      'Ilpo Larha tar en liten lur',
      ':,: Hela gisslan rymmer genom fönstret,',
      'Ilpo Larha vaknar aldrig mer :,:',
      'Anna Lindh shoppar uti NK,',
      'knivman kommer, stabbar Annas buk.',
      ':,: Blod och tarmar spridas över golvet,',
      'Anna Lindh finns ej längre mer. :,:',
      'Liten färja simmar ut från Tallinn,',
      'liten färja börjar sjunka ner.',
      ':,: Det är svart att ändas under vatten,',
      'liten färja simmar aldrig mer. :,:'
    ],
    title: 'Liten Jumbo'
  },
  {
    metadata: ['(Säv. Isoisän olkihattu)'],
    number: '805',
    lyrics: [
      'Mä tässä kerran ullakolle yksin kapusin',
      'ja sattumalta vanhan kaapin siellä aukaisin.',
      'Mä sitä pengoin, mitä lienen oikein etsinyt,',
      'niin löysin vanhan varmuuskumin, siitä kerron nyt.',
      'Sen pölystä kun puhdistin ja sitä kääntelin,',
      'ja ullakolle vanhan arkun päälle istahdin,',
      'en aikaa tiedä, miten kauan siinä viivähdin,',
      'kun isoisän tarinaa mä hiljaa muistelin.',
      "Ol' kerran pieni seksikaupppa sivukadulla,",
      'niin pientä kauppaa nykyään et löydä todella,',
      'Ja siihen puotiin isoisä kerran piipahti,',
      'hän kauan etsi, - varmuuskumin viimein valitsi.',
      'Vaan valinta se tuskin siinä aikaa vienyt ois,',
      'kas, syy on toinen, hennonnut ei millään mennä pois;',
      'kun kerran katsoi myyjättären silmiin sinisiin,',
      'jo kaikki kortsut ostanut hän kohta olis niin.',
      'Näin kului aikaa, - sinisilmäin vuoksi tosiaan,',
      'nyt romanssi niin kaunis kohta puhkes kukkimaan,',
      'ja joka päivä iltaisin, kun kello tuli kuus,',
      'nähtiin eräs herrasmies ja varmuuskumi uus',
      'odottavan sulkemista pienen myymälän,',
      'ja kahden nuoren kulkevan luo puiston hämärän.',
      'Ei kauniimmin tää satu pieni päättyä nyt voi,',
      'kun kera syksyn lehtien - hääkellot heille soi.',
      'Näin kuvat kulki muistossani hämys ullakon,',
      'ja mietin, kuinka kaunis sentään ihmiselo on.',
      'Vaan miten vähän jääkään meistä muistoks tulevain',
      'kuin isoisän tarinasta - varmuuskumi vain.',
      "Mutt' ehkä ajan tomun alta joku toinenkin",
      'joskus pienen muiston löytää niiin kuin minäkin.',
      'Näin isoisän varmuuskumi sai mun laulamaan,',
      'ja vanhan kaapin kätköihin sen laitoin uudestaan.'
    ],
    title: 'Isoisän varmuuskumi'
  },
  {
    metadata: ['(Säv. Sjösala vals)'],
    number: '806',
    lyrics: [
      'Attila nyt ampaisee ylös ratsulleen.',
      'Poskiluut on korkeella, hilpara kourassa.',
      'Raiskaus ja ryöstöt on hällä mielessään,',
      'huutohonsa yhtyvät hunnit satapäin.',
      '>>Hurmeessa hän uittaa jo vihollisiaan,',
      '>>veljensäkin murhauttaa varmuudeksi vaan.',
      '>>Katso, kuinka ruumiita monta on jo aroniityllä:',
      '>>germaani, burgundi, gootti ja moni muu.',
      'Lainehilla Tonavan veri vaahtoaa,',
      'Attila kun orjia seipäille viskoopi.',
      'Attilalle auki on taivas ja maa.',
      'Nylkemälleen kersalle näin karjahtaa:',
      '>>"Tunnetko sä piltti kuka niittäjä on tää?',
      '>>Kuule äitis huutaa kun häntä mäiskitään.',
      '>>Katso, veriloistossaan tervehtii jo aroniityllä:',
      '>>germaani, burgundi, gootti ja moni muu."',
      'Reinille hän ratsastaa hunnilaumoineen,',
      'Suippolakki päässänsä polttaa ja ryöstää hän.',
      "Raa'asti hän mestaa taas ohimennessään,",
      'lapset saavat peliinsä isänsä pään.',
      '>>Katso, isin kallosta potkupallon teen,',
      '>>huutaa lapsi innoissaan rampaäidilleen.',
      '>>Taas niin monta silmääkin puhkottu on aroniityllä:',
      '>>germaani, burgundi, gootti ja moni muu.',
      'Attila on vanha, mutta lahtaa hän vaan.',
      'Siellä miss on kulkenut, kasva ei ruohokaan.',
      'Harvoin hän saa huilia, työtä piisaa ain.',
      'Armoa heikoille ei anneta lain.',
      '>>Tietänee sen frankki, joka miekan vatsaansa sai,',
      '>>skyytti sekä helleeni, Jaappanin samurai.',
      '>>Tietänee sen pääkallot, jotka loistaa aroniityllä:',
      '>>germaani, burgundi, gootti ja moni muu.'
    ],
    title: 'Attilan keväthuumaus'
  },
  {
    metadata: ['(Säv. Lambada)'],
    number: '811',
    lyrics: [
      ':,: Naidaan lampaita, lampailla ei ole hampaita. :,:',
      ':,: Laita lammas rinteeseen, pistä sorkat saappaaseen,',
      'työnnä sormi lampaan perseeseen. :,:',
      ':,: Naidaan lehmiä, lehmät eivät ehdi empiä. :,:',
      ':,: Laita kulli sieraimeen, niin sen kieli lipaisee,',
      'ihanasti sinun kivekseen. :,:',
      ':,: Naidaan norsuja, norsut eivät käytä kortsuja. :,:',
      ':,: Ota pesti sirkukseen, liity ihmeperheeseen,',
      'työnnä jalka norsun perseeseen. :,:',
      ':,: Naidaan poroja, porot ovat metsän horoja. :,:',
      ':,: Tartu kiinni sarvista, ota haju arvesta,',
      'vedä varvi koko tokasta. :,:',
      ':,: Naidaan majavaa, majavalta löytyy parrua. :,:',
      ':,: Ota kiinni hampaista, vedä parru padosta,',
      'työnnä parru pikku majavaan. :,:',
      ':,: Naidaan lääkäärii, lääkäri tietää sinun tautisi. :,:',
      ':,: Ota pistos hauikseen, laita lääkäri kontilleen,',
      'työnnä neula lääkärin perseeseen. :,:'
    ],
    title: 'Naidaan lampaita'
  },
  {
    metadata: [],
    number: '809',
    lyrics: [
      'Kana sanoo kot kot kot kot kot kot.',
      'Kana sanoo kot kot kot kot kot kot.',
      'Kuinka kana sanoo? Kuinka kana sanoo?',
      'Kuinka kana sanoo Cabana?',
      'Kana sanoo kot kot kot kot kot kot.',
      'Karhu sanoo mur mur mur mur mur mur.',
      'Karhu sanoo mur mur mur mur mur mur.',
      'Kuinka karhu sanoo? Kuinka karhu sanoo?',
      'Kuinka karhu sanoo Karhu?',
      'Karhu sanoo mur mur mur mur mur mur.',
      'Gorilla sanoo ärrhg ghräghgrr.',
      'Gorilla sanoo ärrhg ghräghgrr bundolo.',
      'Kuinka gorilla sanoo? Kuinka gorilla sanoo?',
      'Kuinka gorilla sanoo Carillo?',
      'Gorilla sanoo ärrhg ghräghgrr.',
      'Ketulla on krapula, niin myös madolla;',
      'Kaikki ne voit tavata Alkon jonossa!',
      'Kirahvi sanoo öri öri öri öri öri öri.',
      'Kirahvi sanoo öri öri öri öri öri öri.',
      'Kuinka kirahvi sanoo? Kuinka kirahvi sanoo?',
      'Kuinka kirahvi sanoo Kahvi - likööri?',
      'Kirahvi sanoo öri öri öri öri öri öri.',
      'Siili käyttää aineita, niin myös etana,',
      'Heitä et voi tavata Alkon jonossa!',
      'Kiiski sanoo ... ...',
      'Kiiski sanoo ... ...',
      'Kuinka kiiski sanoo? Kuinka kiiski sanoo?',
      'Kuinka kiiski sanoo Whisky?',
      'Kiiski sanoo ... ...',
      'Siittiö sanoo ptlqm ptlqm.',
      'Siittiö sanoo ptlqm ptlqm.',
      'Kuinka siittiö sanoo? Kuinka siittiö sanoo?',
      'Kuinka siittiö sanoo I love you?',
      'Siittiö sanoo ptlqm ptlqm.',
      'ptlqm ptlqm ptlqm ptlqm.'
    ],
    title: 'Onomatopoeettinen alkoholiliike'
  },
  {
    metadata: [],
    number: '810',
    lyrics: [
      'Tyttö näki sillalta kuinka mustaa ',
      'verta valui ranteesta rotvalliin. ',
      'Äiti kuoli Fiskars kurkussa puskaan ',
      'isä teki lääkkeillä itsarin. ',
      'Ja veri oli mustaa, setä kuoli tuskissaan ',
      'ja pappi hukkui sillalta Saimaaseen. ',
      'Veli roikkui rännistä, näin sen itkeneen ',
      'tuli teki helvetin eteiseen. ',
      'Toiset menee kuuhun ja toisen Ruotsiin ',
      'toiset menee Norjaan ja saaristoon. ',
      'Eno kuoli muslimimaassa ruoskiin ',
      'Makekin siel ilman päätä on. ',
      'Ja muija näki Cthulhun, vaari törmäs kaiteeseen ',
      'mummo kaatu suihkussa ammeeseen. ',
      'Veli kiikkuu rännissä vittu edelleen ',
      'joku ampu serkkua pohkeeseen...'
    ],
    title: 'Tuli teki helvetin eteiseen'
  },
  {
    metadata: ['Ja illan viimeinen (vähintään 5 ‰)'],
    number: '000',
    lyrics: [
      'Mä tunnen sen jo kasvavan, voiman sisälläin.',
      'Suuren haasten toteutan ja teen sen pystypäin.',
      'Takaa vuorten huippujen, etsin aina vaan.',
      'Ja pokémonin jokaisen, mä opin tuntemaan.',
      'Pokémon! Omakseni saan (usko vaan)',
      'Me yhdessä voitetaan.',
      'Pokémon! Oot mun ystäväin,',
      'olet aina vierelläin.',
      'Pokémon! Omakseni saan (kun uskaltaa),',
      'niin rohkeus palkan saa.',
      'Toinen toistaan opettaa.',
      'Pookéémoon!',
      'Omakseni saan!',
      'Omakseni saan! Pokémon!!'
    ],

    title: 'Pokémon'
  },

  {
    metadata: ['(Säv. Aikuinen nainen)'],
    number: '001',
    lyrics: [
      'Kännäävä riski mä oon, en enää eksy internettiin.',
      'Kännimme vahvistukoon.',
      'Tahtoisin ottaa, Sotkulla pari kaljaa',
      'kun demot vaikeudet tielemme tuo.',
      'Kaljasta me saamme suojaavan kännin.',
      'Kuolaa tää Karhuni tuo.',
      'Pöhnämme kestää, laatan emme anna estää.',
      'Vähästä luovun,',
      'jos sen päästän viereen puun.',
      'Eeeee-en juomiasi tahdo riistää.',
      'Tahdo eeeeeen, selvyyttäsi kiistää.',
      'Sua tarviteeeeen, ryyppykaveriksi iiiii-kuisen riskin.',
      'Kaiken mu juomme, toisiamme tukekaamme.',
      'Sotkulle suojaan, sinut vien jos vielä osaan.',
      'Pullooni sulje mä en.',
      'Kaljastani taistelen enkä pelkää.',
      'Karhusta huolehtien,',
      'kestämme paineet, meitä eivät paina huolet.',
      'Sotkulle suojaan, Karhun vien jos viel osaan.',
      'Eeeee-en juomiasi tahdo riistää.',
      'Tahdo eeen, selvyyttäsi kiistää.',
      'Sua tarvitseeeen, ryyppykaveriksi iiii-kuisen riskin.',
      'Kaiken me juomme, toisiamme tukekaamme.',
      'Ikuinen riski, tuntee arvon viinasten.'
    ],

    title: 'Ikuinen riski'
  },
  {
    metadata: ['(Esit. Eppu Normaali)'],
    number: '327',
    lyrics: [
      'Poliisi ajaa sinisellä autolla, uaaa,',
      'sinisellä autolla.',
      'Poliisilla on pillit katolla, uaaa,',
      'pillit katolla.',
      'Poliisi pamputtaa kumisella pampulla, uaaa,',
      'kumisella pampulla.',
      'Poliisi uhkaa ihmisiä putkalla, uaaa,',
      'ihmisiä putkalla.',
      'On mukavaa ajaa sinisellä autolla, uaaa,',
      'sinisellä autolla.',
      'Mukavaa pamputtaa kumisella pampulla, uaaa,',
      'kumisella pampulla.',
      'Systeemin vartija se poliisin on työ.',
      'Poliisi pilttejä pampulla lyö.',
      'Toista kaikki alusta alkaen'
    ],
    title: 'Poliisi pamputtaa taas'
  },
  {
    metadata: [
      '(Tunnettu myös nimellä Kauppaneuvos Paukun juomalaulu tai Professori Järven juomalaulu)'
    ],
    number: '114',
    lyrics: [
      ':,: Henkilökuntaa, henkilökuntaa, parlevuu! :,:',
      'Henkilökuntaa, henkilökuntaa, henkilökuntaa, henkilökuntaa!',
      ':,: Henkilökuntaa, parlevuu! :,:',
      'Tärpättiä...',
      'Konjakki ois poikaa...',
      'Sikaria...',
      'Kaljakin kelpaa...',
      'Lissää viinaa...',
      'Lasolia...',
      'Aspiriinia...'
    ],
    title: 'Henkilökuntaa'
  },
  {
    metadata: ['(Esit. Kaija Koo)'],
    number: '340',
    lyrics: [
      'Hei, ei ei, en sitä salaa,',
      'näillä teillä loppuun palaa,',
      'tinakenkäni on sulaa laavaa.',
      'Hän katselee rauhattomana itään,',
      'tuo tyttö hentoinen tinakengissään.',
      'Kuuluuko sieltä suunnalta ketään,',
      'joka voisi ottaa tytön reppuselkään?',
      'Hän on pitänyt viestin karttaan,',
      'siellä lukee vie minut turvaan.',
      'Ehkä hänet on luotu lentämään,',
      'ettei sulata tie hänen kenkiään.',
      '__KERTO:__',
      '>>Hei, ei ei en sitä salaa,',
      '>>näillä teillä loppuun palaa,',
      '>>tinakenkäni on sulaa laavaa.',
      '>>Tule, tule siis ennen aamua',
      '>>ethän sä muuten enää löydä mua',
      '>>kun aurinko nousee on pakko liikkuma.',
      'Hän muistaa kuinka koulussa naurettiin,',
      'kun hän oli toisenlainen kuin kaikki muut.',
      'Ja unissaan hän palaa pulpettiin,',
      'ja tinasotilasarmeija tukkii toisten suut.',
      'Ei täällä voi olla se paikoillaan,',
      'muuten tinakenkäsi sulaa. (Whooo-o-wooo, ooo-oo!)',
      'Juokse sinne missä on viileää,',
      'siellä Amor nuolella tähtää. (Whooo-o-wooo, ooo-oo!)',
      '__KERTO__',
      'Tämä tie sut loppuun käyttää,',
      'sen pinta väreilee.',
      'Kaikki kangastuksilta näyttää',
      'ja ilma syttyy tuleen.',
      'Pojan tinakenkäisen,',
      'mistä löytäisit sä sen, tyttö tinakenkäinen.',
      'KERTO'
    ],
    title: 'Tinakenkätyttö'
  },
  {
    metadata: ['(Tunnetaan myös nimellä Kansainvälinen)'],
    number: '524',
    lyrics: [
      'Mera brännvin i glasen,',
      'mera glas på vårt bord,',
      'mera bord på kalasen,',
      'mera kalas på vår jord.',
      'Mera jordar kring månen,',
      'mera månar kring Mars',
      'mera marscher till Skåne,',
      'mera Skåne, bevars, bevars, bevars!',
      'Lisää viinaa mun lasiin,',
      'lisää laseja pöydälle,',
      'lisää pöytiä näihin juhliin,',
      'lisää juhlia kansalle.',
      'Lisää kansaa Suomeen,',
      'lisää Suomea päälle maan,',
      'lisää maata Suur-Suomelle,',
      'marssitaan, marssitaan, Karjalaan, Karjalaan!'
    ],
    title: 'Internationalen'
  },
  {
    metadata: ['(Kalle-Kustaan muori)'],
    number: '525',
    lyrics: [
      'Hitihatit työnnetään hissikuiluihin. x3',
      'Nyt on aika muutoksen!',
      'Demareiden touhu on kuin Linnanmäeltä. x3',
      'Nyt on aika muutoksen!',
      'Vassareiden paikka löytyy Töölönlahdesta. x3',
      'Nyt on aika muutoksen!',
      'Vihreät ne suljetaan biolaitokseen x3',
      'Nyt on aika muutoksen!',
      'Lannanhaju leviääpi Kepun suunnasta. x3',
      'Nyt on aika muutoksen!',
      'Kitkekää nyt kokkarit pois ruispellosta x3',
      'Nyt on aika muutoksen!',
      'Kristilliset palaa kirkkojensa mukana x3',
      'Nyt on aika muutoksen!',
      'Svenskagruppen lastataan me ruotsinlaivoihin. x3',
      'Nyt on aika muutoksen!',
      'Perussuomalaiset löytää raastuvasta. x3',
      'Nyt on aika muutoksen!',
      'Piraattien laivat uppoo Somaliassa. x3',
      'Nyt on aika muutoksen!'
    ],
    title: 'Oodi sitoutumattomuudelle'
  }
];
